...
async function isDemoColAvailable(): Promise<boolean> {
  ...
  try {
    const r = await sbAdmin("game_bets?select=is_demo&limit=0");
...
// ── Helper: obtener perfil del usuario — 30s per-user cache ──────────────────
const _getProfileCache = new Map<string, { data: { mander_id: string; username: string; balance?: number; is_blocked?: boolean } | null; at: number }>();
const GET_PROFILE_CACHE_TTL = 30_000;

async function getProfile(userId: string): Promise<{ mander_id: string; username: string; balance?: number; is_blocked?: boolean } | null> {
  const cached = _getProfileCache.get(userId);
  if (cached && Date.now() - cached.at < GET_PROFILE_CACHE_TTL) return cached.data;
  const r = await sbAdmin(`profiles?id=eq.${userId}&select=mander_id,username,balance,is_blocked&limit=1`);
  const rows = await r.json();
  const data = rows?.[0] ?? null;
  _getProfileCache.set(userId, { data, at: Date.now() });
  return data;
}
...
router.get("/transactions", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    let txs: any[] = await txRes.json() ?? [];
    console.log(`[TX GET] user=${req.authUser!.id} → ${txs.length} transacciones`);

    // ── Enrich deposit transactions with blockchain hash from deposits table ──
    // When the Plisio webhook fires before the blockchain hash is available, the
    // transaction record gets external_tx_id = Plisio invoice ID (not a real hash).
    // We do a read-only lookup of deposits.tx_hash to fill in the gap.
    const isBlockchainHash = (h: string) =>
      /^0x[0-9a-fA-F]{40,}/i.test(h) || /^[0-9a-fA-F]{64}$/i.test(h);

    const depositTxsNeedingHash = txs.filter(
      (t: any) =>
        t.type === "deposit" &&
        t.status === "completed" &&
        (!t.external_tx_id || !isBlockchainHash(String(t.external_tx_id))),
    );

    if (depositTxsNeedingHash.length > 0) {
      // Extract deposit IDs from notes (formats: "deposit_id:123", "dep=123 ")
      const depIdMap = new Map<string, string>(); // depositId → txId
      for (const tx of depositTxsNeedingHash) {
        const notes = String(tx.notes ?? "");
        const m = notes.match(/deposit_id:(\w+)/) ?? notes.match(/\bdep=(\w+)/);
        if (m) depIdMap.set(m[1], tx.id);
      }

      if (depIdMap.size > 0) {
        const ids = Array.from(depIdMap.keys()).join(",");
        const depRes = await sbAdmin(
          `deposits?id=in.(${ids})&select=id,tx_hash`,
          { headers: { Prefer: "count=none" } },
        ).catch(() => null);

        if (depRes?.ok) {
          const deps: { id: string; tx_hash: string | null }[] = await depRes.json().catch(() => []);
          const depHashMap = new Map(
            deps
              .filter((d: any) => d.tx_hash && isBlockchainHash(String(d.tx_hash)))
              .map((d: any) => [String(d.id), d.tx_hash as string]),
          );
          // Enrich transactions with the real blockchain hash
          if (depHashMap.size > 0) {
            txs = txs.map((tx: any) => {
              if (tx.type !== "deposit") return tx;
              const notes = String(tx.notes ?? "");
              const m = notes.match(/deposit_id:(\w+)/) ?? notes.match(/\bdep=(\w+)/);
              const depId = m?.[1];
              const realHash = depId ? depHashMap.get(depId) : undefined;
              if (!realHash) return tx;
              return { ...tx, deposit_tx_hash: realHash };
            });
          }
        }
      }
...
// Per-user 12s cache to avoid re-fetching all game_bets on every poll.
// ──────────────────────────────────────────────────────────────────────────────
const _profileCache = new Map<string, { data: unknown; at: number }>();
const PROFILE_CACHE_TTL = 30_000;
export function invalidateProfileCache(userId: string) { _profileCache.delete(userId); }
...
router.get("/profile", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Capturar IP del usuario activo (solo escribe a disco si cambia algo)
    const rawIp = (req.headers["x-forwarded-for"] as string)
      || (req.headers["x-real-ip"] as string)
      || (req as any).ip
      || (req as any).socket?.remoteAddress
      || "";
    recordIp(userId, profileRow.username || username, rawIp);
    ...
    try {
      ...
      // 4 queries in parallel: balances, cycle transactions, shared bet cache, affiliate
      // Shared bet cache (5-min TTL) avoids per-user bets being capped at 1000 rows
      const [balRes, txRes, allBetsCache, refRes] = await Promise.all([
        sbAdmin(`balances?mander_id=eq.${mid}&select=balance`, { headers: { Prefer: "count=none" } }),
        sbAdmin(`transactions?mander_id=eq.${mid}&type=in.(deposit,wager_reset)&select=type,amount,currency,status,created_at&order=created_at.asc&limit=2000`, { headers: { Prefer: "count=none" } }),
        getCachedBetRows(),
        sbAdmin(`affiliate_referrals?referred_username=ilike.${encodeURIComponent(profileRow.username)}&select=referrer_username&limit=1`, { headers: { Prefer: "count=none" } }).catch(() => null),
      ]);
      ...
      let cycleSince: Date | null = null;
      if (txRes.ok) {
        const txRows: { type: string; amount: number; currency: string; status: string; created_at: string }[] = await txRes.json();

        // Find the latest wager_reset marker to determine current cycle start
        for (const t of txRows) {
          if (t.type === "wager_reset") {
            const d = new Date(t.created_at);
            if (!cycleSince || d > cycleSince) cycleSince = d;
          }
        }

        // Sum only deposits from current cycle (after latest wager_reset, or all-time if none)
        totalDeposit = txRows
          .filter(t =>
            t.type === "deposit" &&
            (t.status === "completed" || t.status === "confirmed") &&
            (cycleSince === null || new Date(t.created_at) > cycleSince)
          )
          .reduce((s, t) => s + txToUsd(t), 0);
      }

      // Filter shared cache to this user's bets only
      const usernameLowerTx = username.toLowerCase();
      const allBetRowsUser = allBetsCache.filter(
        (r: any) => (r.username ?? "").toLowerCase() === usernameLowerTx
      );
      const cycleRows = cycleSince
        ? allBetRowsUser.filter((r: any) => new Date(r.created_at) > cycleSince!)
        : allBetRowsUser;
      wageredTotal = Math.round(cycleRows.reduce((s, r) => s + Math.abs(parseFloat(String(r.bet_usd || 0))), 0) * 100) / 100;
      console.log(`[PROFILE] username=${username} cycle_since=${cycleSince?.toISOString() ?? "all"} wagered_total=${wageredTotal} total_deposit_usd=${Math.round(totalDeposit*100)/100}`);
    ...
    const profileData = {
      profile: {
        ...
        balance:      realBalance,
        balance_demo: (() => {
          const loc   = Number(authMeta.balance_demo_local ?? 0);
          const grant = Number(authMeta.balance_demo ?? 0);
          return loc > 0 ? loc : grant;
        })(),
    ...
    _profileCache.set(userId, { data: profileData, at: Date.now() });
...
router.get("/balance", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const authMeta = authUserRes.ok ? ((await authUserRes.json())?.app_metadata ?? {}) : {};
    // balance_demo       = admin-authorized amount (deducted on withdrawal, restored on rejection).
    // balance_demo_local = client's real-time playing balance (synced from frontend; 0 when unset).
    //
    // Rules:
    //  • If balance_demo_local > 0 (user has been playing and has a real synced value):
    //      cap at balance_demo so a pending withdrawal deduction is always visible even if the
    //      client sync re-writes balance_demo_local to the pre-withdrawal value.
    //  • If balance_demo_local == 0 (never set, or drained to 0 in a previous session):
    //      fall back to balance_demo (the admin grant) so fresh balances are shown correctly.
    const rawDemoLocal = Number(authMeta.balance_demo_local ?? 0);
    const rawDemoGrant = Number(authMeta.balance_demo ?? 0);
    const balanceDemo  = rawDemoLocal > 0 ? rawDemoLocal : rawDemoGrant;
...
router.post("/balance/sync", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Update profiles.balance by user_id (avoids slow getProfile() lookup).
    ...
    // If no valid totalUsd provided, skip the update entirely — balance poll handles corrections.

    console.log(`[BALANCE SYNC] user=${userId} ${cur}=${newBal} totalUsd=${totalUsd ?? "skipped"}`);
    return res.json({ ok: true });
  } catch (err: any) {
...
function scheduleSimBet() {
  const delay = 15000 + Math.floor(Math.random() * 20000); // 15–35 s
  setTimeout(() => {
...
router.get("/top-bets", async (_req: Request, res: Response) => {
  ...
  const pickCurrency = (dbCurrency: string | undefined, seed: string): string => {
    // Preserve the actual currency stored in the DB (including USDT).
    // Only fall back to a seeded-random coin for legacy "USD" records or
    // entries that have no currency at all (e.g., old rows before the column existed).
    if (dbCurrency && dbCurrency !== "USD") return dbCurrency;
...
router.get("/live-bets", async (_req: Request, res: Response) => {
  ...
  if (realInBuffer < 20) {
    try {
      ...
      if (dbRes.ok) {
        ...
        // Dedup key uses amounts (rounded to cents) + game instead of created_at,
        // because the buffer timestamp (set by Node) and the DB timestamp (set by Postgres)
        // differ by milliseconds, causing the same bet to appear twice.
        const bufKeys = new Set(bufferEntries.map(b =>
          `${b.username}_${b.game}_${Math.round(b.bet_usd * 10000)}_${Math.round(b.payout_usd * 10000)}`
        ));
        dbRows = rows
          .filter(r => !HIDDEN_FROM_BETS.has(r.username.toLowerCase()))
          .filter(r => !bufKeys.has(`${r.username}_${r.game}_${Math.round(Number(r.bet_usd) * 10000)}_${Math.round(Number(r.payout_usd) * 10000)}`))
          .map(r => ({
            username:   r.username,
            game:       r.game,
  ...
  // Merge buffer + DB rows, sort newest-first (excluir usuarios ocultos).
  // Use created_at in the final dedup key so multiple same-amount bets placed at
  // different times are NOT collapsed into one (the old amounts-only key caused
  // rapid repeated bets to disappear from the feed).
  // The buffer+DB cross-dedup above (bufKeys) already handled the case where the
  // same physical bet appears in both sources with slightly different timestamps.
  const seen = new Set<string>();
  const merged = [...bufferEntries, ...dbRows].filter(b => {
    if (HIDDEN_FROM_BETS.has(b.username.toLowerCase())) return false;
    const k = `${b.username}_${b.game}_${Math.round(b.bet_usd * 10000)}_${Math.round(b.payout_usd * 10000)}_${b.created_at}`;
...
router.post("/bet-result", requireAuth, async (req: Request, res: Response) => {
  ...
  // No dedup on the live feed buffer: idempotency (request_id) already prevents
  // double balance delta in the DB. Every valid bet should appear in the live feed.
  if (!HIDDEN_FROM_BETS.has(username.toLowerCase())) {
    ...
    const VALID_CURRENCIES = new Set(["USDT","USDC","BTC","ETH","BNB","SOL","TRX"]);
    const displayCurrency = VALID_CURRENCIES.has(String(currency).toUpperCase()) ? String(currency).toUpperCase() : "USDT";
    liveBetsBuffer.push({ username, game: gameStr, bet_usd, payout_usd: displayPayout, multiplier, win, created_at: new Date().toISOString(), currency: displayCurrency });
  ...
  try {
    ...
    const userId = req.authUser!.id;
    const [refRes, profRes, statsRes, authUserRes] = await Promise.all([
      sbAdmin(
      ...
      // Fetch app_metadata to get authoritative demo balance (server-side demo detection)
      fetch(`${SUPABASE_URL}/auth/v1/admin/users/${encodeURIComponent(userId)}`, {
        headers: { apikey: SUPABASE_SERVICE_KEY!, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}` },
      }).catch(() => null),
    ]);
    ...
    // Server-side demo detection: check balance_demo_local in app_metadata.
    // Only use balance_demo_local (the live synced value) — do NOT fall back to balance_demo (the
    // original admin grant amount), because that value is never zeroed out and would permanently
    // mark all future real bets as demo for any user who ever received a demo grant.
    const authMetaRaw = (authUserRes && authUserRes.ok) ? ((await authUserRes.json())?.app_metadata ?? {}) : {};
    const balanceDemoLocal  = authMetaRaw.balance_demo_local;
    const serverDemoBalance =
      balanceDemoLocal !== null && balanceDemoLocal !== undefined && Number(balanceDemoLocal) > 0
        ? Number(balanceDemoLocal)
        : 0;
    // A user is in demo mode if client explicitly flagged it OR server confirms active demo balance.
    const isDemoBet = is_demo === true || serverDemoBalance > 0;
    console.log(`[BET-RESULT-DEBUG] username=${username} game=${game} is_demo_client=${JSON.stringify(is_demo)} balance_demo_local=${balanceDemoLocal} serverDemoBalance=${serverDemoBalance} isDemoBet=${isDemoBet}`);
    ...
    if (demoCol) betRow.is_demo = isDemoBet;
...
// GET /api/my-bets — bet history from game_bets
// Supports two modes:
//   Offset mode (bet history pagination):  ?offset=0&limit=30&game=keno
//   Cursor mode (initial profile load):    ?limit=50&cursor=<iso>
// Response: { bets, total, nextCursor?, gameSummary }
const _myBetsCache = new Map<string, { data: unknown; at: number }>();
const MY_BETS_CACHE_TTL = 20_000;

router.get("/my-bets", requireAuth, async (req: Request, res: Response) => {
  const userId = req.authUser!.id;
  const myBetsCacheKey = `${userId}:${JSON.stringify(req.query)}`;
  const mbc = _myBetsCache.get(myBetsCacheKey);
  if (mbc && Date.now() - mbc.at < MY_BETS_CACHE_TTL) return res.json(mbc.data);
  ...
  const useOffset = req.query.offset !== undefined;

  if (useOffset) {
    // ── Offset-based mode (bet history page navigation) ──────────────────────
    const limit  = Math.min(Math.max(1, parseInt(req.query.limit  as string) || 30), 100);
    const offset = Math.max(0, parseInt(req.query.offset as string) || 0);
    const game   = typeof req.query.game === "string" && req.query.game && req.query.game !== "all"
      ? req.query.game.toLowerCase() : null;

    try {
      let qs = `game_bets?select=bet_usd,payout_usd,game,created_at&username=ilike.${encodeURIComponent(username)}&order=created_at.desc&limit=${limit}&offset=${offset}`;
      if (game) qs += `&game=ilike.${encodeURIComponent(game)}`;

      const r = await sbAdmin(qs, { method: "GET", headers: { Prefer: "count=exact" } });
      if (!r.ok) return res.json({ bets: [], total: 0 });
      ...
      if (cr) {
        ...
        if (totalStr && totalStr !== "*") total = parseInt(totalStr) || 0;
      }
      ...
      const mbOffsetData = { bets, total };
      _myBetsCache.set(myBetsCacheKey, { data: mbOffsetData, at: Date.now() });
      return res.json(mbOffsetData);
    } catch (e: any) {
      return res.status(500).json({ error: e.message });
    }
  }

  // ── Cursor-based mode (initial profile load / legacy) ────────────────────
  ...
  try {
    ...
    const r = await sbAdmin(qs, { method: "GET", headers: { Prefer: cursor ? "count=none" : "count=exact" } });
    if (!r.ok) return res.json({ bets: [], nextCursor: null, gameSummary: [], total: 0 });
    const rows: any[] = await r.json();
    ...
    const mbCursorData = { bets, nextCursor, gameSummary, total };
    _myBetsCache.set(myBetsCacheKey, { data: mbCursorData, at: Date.now() });
    return res.json(mbCursorData);
  } catch (e: any) {
    return res.status(500).json({ error: e.message });
  }
});
...
router.get("/stats", requireAuth, async (req: Request, res: Response) => {
  ...
  try {
    // Supabase/PostgREST default max-rows is 1000 per request regardless of
    // the &limit= param.  Paginate until we have ALL bets so that total_bets,
    // game_summary, wagered, etc. are never silently capped.
    const PAGE_SIZE = 1000;
    const rows: any[] = [];
    let offset = 0;
    while (true) {
      const r = await sbAdmin(
        `game_bets?username=ilike.${encodeURIComponent(username)}&select=bet_usd,payout_usd,game,currency,created_at&order=created_at.desc&limit=${PAGE_SIZE}&offset=${offset}`,
        { headers: { Prefer: "count=none" } },
      );
      ...
      offset += PAGE_SIZE;
      if (rows.length >= 100_000) break;         // safety cap
    }
