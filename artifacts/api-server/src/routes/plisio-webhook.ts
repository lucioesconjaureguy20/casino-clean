...
import { creditBalanceNative, atomicProfileBalanceDelta, getBalanceUsd, insertWagerReset, WAGER_CYCLE_RESET_THRESHOLD_USD } from "../lib/atomicBalance";
import { sendTelegramDeposit, getRefTag } from "../lib/telegram.js";
import { nextDepositDisplayId } from "../lib/counters";
...
async function addToCryptoBalance(manderId: string, userId: string, cryptoAmount: number, currency: string, deltaUsd: number) {
  ...
  // 1. Atomic native balance credit — SELECT FOR UPDATE + UPDATE SET balance = balance + delta.
  const result = await creditBalanceNative(manderId, cur, cryptoAmount, userId);
  if (result.ok) {
    console.log(`[Plisio] balances +${cryptoAmount} ${cur} mander=${manderId} new=${result.newBalance}`);
  } else {
    console.error(`[Plisio] creditBalanceNative failed: ${result.error} mander=${manderId}`);
  }

  // 2. Atomic additive UPDATE on profiles.balance (display cache).
  // Uses PostgreSQL RPC → UPDATE SET balance = GREATEST(0, balance + delta)
  // Eliminates the read-modify-write race from the old READ+COMPUTE+WRITE pattern.
  await atomicProfileBalanceDelta(manderId, deltaUsd);
  console.log(`[Plisio] profiles.balance Δ${deltaUsd} USD mander=${manderId}`);
}
...
async function insertTransaction(
  ...
  // Dedup: si ya existe una TX para este deposit_id (creada por webhook O poller), no insertar otra
  const dupCheck = await sbAdmin(
    `transactions?or=(notes.like.*deposit_id:${encodeURIComponent(depositId)}*,notes.like.*dep=${encodeURIComponent(depositId)} *)&type=eq.deposit&limit=1`,
    { headers: { Prefer: "count=none" } },
  ).catch(() => null);
  if (dupCheck?.ok) {
    const existing = await dupCheck.json().catch(() => []);
    if (Array.isArray(existing) && existing.length > 0) {
      console.log(`[Plisio] insertTransaction SKIP — ya existe TX para deposit_id=${depositId} (id=${existing[0].id})`);
      return false;
    }
  }
  ...
  const usdNote = deltaUsd != null && deltaUsd > 0 ? ` usd:${deltaUsd.toFixed(2)}` : "";
  const r = await sbAdmin("transactions", {
    ...
    body: JSON.stringify({
      ...
      status:         "completed",
      external_tx_id: txId,
      notes:          `plisio:${txId} deposit_id:${depositId}${notesAddr}${notesHash}${usdNote}`,
      completed_at:   new Date().toISOString(),
    }),
  });
  if (!r.ok) console.error("[Plisio] error insertando transaction:", await r.text());
  else console.log(`[Plisio] tx registrada: ${amount} ${currency} (~$${deltaUsd?.toFixed(2) ?? "?"}) mander=${manderId} display_id=${displayId}`);
  return r.ok;
}
...
async function handlePlisioWebhook(req: Request, res: Response) {
  ...
  // NOTE: source_amount de Plisio está en la moneda del pago (ej: TRX), NO en USD.
  // Para USDT funciona de casualidad (USDT ≈ $1). Para TRX/BNB/etc. sería incorrecto usarlo como USD.
  // → siempre calculamos USD desde el precio live.
  ...
  console.log(`[Plisio webhook] status=${status} order=${orderNumber} txn=${txnId} crypto=${cryptoAmt}`);
  ...
  if (status === "pending") {
    // "pending" = TX vista en mempool, aún NO confirmada en blockchain.
    // Actualizamos el monto en DB (para badge ⏳ en UI) pero NUNCA acreditamos —
    // esperamos el webhook "completed" o "mismatch" (confirmación real on-chain).
    if (orderNumber && cryptoAmt > 0) {
      ...
      console.log(`[Plisio webhook] pending: badge actualizado dep=${orderNumber} crypto=${cryptoAmt} — esperando completed para acreditar`);
    }
    return res.status(200).json({ message: "ok" });
  }
  ...
  // 7. Calcular USD desde precio live (source_amount de Plisio es en cripto, no en USD)
  ...
  // 9. Verificar si el saldo previo era ~$0 → nuevo ciclo de wagering
  const balBeforeWebhook = await getBalanceUsd(profile.mander_id);
  if (balBeforeWebhook < WAGER_CYCLE_RESET_THRESHOLD_USD) {
    await insertWagerReset(profile.mander_id, deposit.user_id);
  }

  // 10. Acreditar balance
  await addToCryptoBalance(profile.mander_id, deposit.user_id, cryptoAmt, currency, deltaUsd);  // FIX 1

  // 10. Registrar transacción — amount en cripto (igual que deposit.ts y poller)
  // El wagering check multiplica amount × getPriceUsd(currency), así que DEBE ser el monto cripto.
  await insertTransaction(
    profile.mander_id, deposit.user_id,
    cryptoAmt, currency, network,
    txHash, deposit.id, deposit.address,
    deltaUsd,
  );

  // 11. Notificar al grupo de Telegram de depósitos
  try {
    const refTag = await getRefTag(profile.username);
    await sendTelegramDeposit(
      `💸 <b>Nuevo depósito confirmado</b>\n` +
      `👤 Usuario: <b>${profile.username}</b>${refTag}\n` +
      `💰 Monto: <b>${cryptoAmt} ${currency}</b> (~$${deltaUsd.toFixed(2)} USD)\n` +
      `🌐 Red: <b>${network ?? "—"}</b>\n` +
      `🆔 ID: <code>${deposit.id}</code>`,
    );
  } catch (_e) {}

  // 12. Estadísticas de afiliados
  try {
