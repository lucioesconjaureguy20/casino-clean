...
import { creditBalanceNative, atomicProfileBalanceDelta, getBalanceUsd, insertWagerReset, WAGER_CYCLE_RESET_THRESHOLD_USD } from "../lib/atomicBalance.js";
import { sendTelegramDeposit, getRefTag } from "../lib/telegram.js";
...
router.post("/deposit/confirm", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 4. Acreditar balance en tabla balances (operación atómica — sin race condition)
    await creditBalanceNative(profile.mander_id, deposit.currency, confirmedAmount, deposit.user_id);

    // 5. Actualizar profiles.balance (operación atómica — sin race condition)
    const priceUsd = getPriceUsd(deposit.currency.trim().toUpperCase());
    const deltaUsd = confirmedAmount * priceUsd;
    await atomicProfileBalanceDelta(profile.mander_id, deltaUsd);
    ...
    // 7. Notificar al grupo de Telegram de depósitos
    try {
      const priceUsdConf = getPriceUsd(deposit.currency.trim().toUpperCase());
      const deltaUsdConf = confirmedAmount * priceUsdConf;
      const refTagConf = await getRefTag(profile.username);
      await sendTelegramDeposit(
        `💸 <b>Nuevo depósito confirmado</b>\n` +
        `👤 Usuario: <b>${profile.username}</b>${refTagConf}\n` +
        `💰 Monto: <b>${confirmedAmount} ${deposit.currency.trim().toUpperCase()}</b> (~$${deltaUsdConf.toFixed(2)} USD)\n` +
        `🌐 Red: <b>${deposit.network ?? "—"}</b>\n` +
...
router.post("/deposit/plisio", requireAuth, async (req: Request, res: Response) => {
  ...
  const { currency, network, request_id } = req.body ?? {};
  const authUser = (req as any).authUser;
  ...
  // Idempotency gate (fail-open — si el RPC no existe aún, continúa de todas formas)
  try {
    const idem = await tryIdempotency(`deposit:plisio:${authUser.id}:${request_id}`, authUser.id);
    if (idem.isDuplicate) {
      return res.status(409).json({ error: "Duplicate request. This deposit has already been submitted.", code: "duplicate_request" });
    }
  } catch (e: any) {
    // No bloqueamos: la función try_idempotency puede no existir todavía en Supabase.
    // El duplicado real lo evita Plisio devolviendo la misma invoice para el mismo request_id.
    console.warn("[Plisio deposit] idempotency check unavailable (fail-open):", e.message);
  }
...
// ── POST /api/deposit/auto-confirm/:id ─────────────────────────────────────────
// Confirma automáticamente un depósito pendiente después de un delay cliente (5-10s).
// Solo el dueño del depósito puede llamarlo.
// Idempotente: si ya fue confirmado (webhook/poller), devuelve ok=true sin doble acreditación.
// Protección doble-crédito: PATCH usa &status=eq.pending — si ya cambió, 0 rows → abort.
router.post("/deposit/auto-confirm/:id", requireAuth, async (req: Request, res: Response) => {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY)
    return res.status(503).json({ error: "Servicio no disponible." });
  ...
  try {
    ...
    const depRes = await fetch(
      `${SUPABASE_URL}/rest/v1/deposits?id=eq.${encodeURIComponent(depositId)}&select=*&limit=1`,
      { headers: { apikey: SUPABASE_SERVICE_KEY!, Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`, Prefer: "count=none" } },
    );
    if (!depRes.ok) return res.status(500).json({ error: "Error leyendo depósito." });
    const [deposit] = await depRes.json();

    if (!deposit) return res.status(404).json({ error: "Depósito no encontrado." });

    // 2. Verificar ownership
    if (deposit.user_id !== userId) return res.status(403).json({ error: "Sin acceso." });

    // 3. Idempotente: ya confirmado → OK sin hacer nada
    if (deposit.status === "confirmed" || deposit.status === "completed") {
      return res.json({ ok: true, already_confirmed: true, amount: parseFloat(String(deposit.amount ?? 0)), currency: deposit.currency });
    }

    // 4. Validar que el monto ya fue registrado por Plisio
    const cryptoAmt = parseFloat(String(deposit.amount ?? 0));
    if (!cryptoAmt || cryptoAmt <= 0) {
      return res.status(400).json({ error: "El depósito aún no tiene monto registrado. Esperá a que Plisio lo detecte." });
    }
    ...
    const now      = new Date().toISOString();
    const fakeHash = `auto_${depositId}_${Date.now()}`;
    const cur      = (deposit.currency ?? "").trim().toUpperCase();

    // 6. PATCH atómico: solo si sigue pending — previene doble crédito ante webhook concurrente
    const patchRes = await fetch(
      `${SUPABASE_URL}/rest/v1/deposits?id=eq.${encodeURIComponent(depositId)}&status=eq.pending`,
      {
        method:  "PATCH",
        headers: {
          apikey:        SUPABASE_SERVICE_KEY!,
    ...
    const patchBody = patchRes.ok ? await patchRes.json().catch(() => []) : [];
    if (!Array.isArray(patchBody) || patchBody.length === 0) {
      // Webhook o poller ya lo confirmó antes que nosotros — idempotente
      console.log(`[auto-confirm] dep=${depositId} ya confirmado por webhook concurrente — ok`);
      return res.json({ ok: true, already_confirmed: true, amount: cryptoAmt, currency: cur });
    }

    console.log(`[auto-confirm] dep=${depositId} amount=${cryptoAmt} ${cur} user=${userId}`);

    // 7. Verificar si el saldo previo era ~$0 → nuevo ciclo de wagering
    const balanceBeforeUsd = await getBalanceUsd(profile.mander_id);
    if (balanceBeforeUsd < WAGER_CYCLE_RESET_THRESHOLD_USD) {
      await insertWagerReset(profile.mander_id, userId);
    }

    // 8. Acreditar balance nativo (atómico, SELECT FOR UPDATE)
    await creditBalanceNative(profile.mander_id, cur, cryptoAmt, userId);

    // 8. Actualizar profiles.balance (caché de display)
    const priceUsd = getPriceUsd(cur);
    const deltaUsd = cryptoAmt * priceUsd;
    await atomicProfileBalanceDelta(profile.mander_id, deltaUsd);

    // 9. Registrar en historial de transacciones
    await insertTransaction(
      profile.mander_id,
      userId,
      cryptoAmt,
    ...
    // 10. Notificar al grupo de Telegram de depósitos
    try {
      const priceUsdAuto = getPriceUsd(cur);
      const deltaUsdAuto = cryptoAmt * priceUsdAuto;
      const refTagAuto = await getRefTag(profile.username);
      await sendTelegramDeposit(
        `💸 <b>Nuevo depósito confirmado</b>\n` +
        `👤 Usuario: <b>${profile.username}</b>${refTagAuto}\n` +
        `💰 Monto: <b>${cryptoAmt} ${cur}</b> (~$${deltaUsdAuto.toFixed(2)} USD)\n` +
        `🌐 Red: <b>${deposit.network ?? "—"}</b>\n` +
        `🆔 ID: <code>${depositId}</code>`,
      );
    } catch (_e) {}

    return res.json({ ok: true, amount: cryptoAmt, currency: cur });
  } catch (err: any) {
    console.error("[auto-confirm] catch:", err.message);
    return res.status(500).json({ error: err.message });
  }
});
...
const _pendingBalCache = new Map<string, { data: unknown; at: number }>();
const PENDING_BAL_TTL = 20_000;

router.get("/deposits/pending-balance", requireAuth, async (req: Request, res: Response) => {
  const userId = (req as any).authUser?.id as string;

  const pbc = _pendingBalCache.get(userId);
  if (pbc && Date.now() - pbc.at < PENDING_BAL_TTL) return res.json(pbc.data);
  ...
  try {
    ...
    const [rPending, rRecent] = await Promise.all([
      fetch(
      ...
      fetch(
        `${SUPABASE_URL}/rest/v1/deposits?user_id=eq.${encodeURIComponent(userId)}&amount=gt.0&select=id,amount,currency,network,status,created_at,confirmed_at&order=created_at.desc&limit=20`,
        { headers },
      ),
    ]);
    ...
    _pendingBalCache.set(userId, { data: pbData, at: Date.now() });
    return res.json(pbData);
  } catch (e: any) {
