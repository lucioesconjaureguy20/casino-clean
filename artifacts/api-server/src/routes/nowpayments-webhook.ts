...
async function insertTransaction(
  ...
  const r = await sbAdmin("transactions", {
    ...
    body: JSON.stringify({
      ...
      notes:          `nowpayments:${paymentId} deposit_id:${depositId}${usdNote}`,
...
router.post("/webhooks/nowpayments", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 4. Verificar si el saldo previo era ~$0 → nuevo ciclo de wagering
    const balBeforeNP = await getBalanceUsd(profile.mander_id);
    if (balBeforeNP < WAGER_CYCLE_RESET_THRESHOLD_USD) {
      await insertWagerReset(profile.mander_id, deposit.user_id);
    }

    // 5. Acreditar balance en tabla balances (solo si el PATCH anterior acreditó exactamente 1 fila)
    await creditBalance(profile.mander_id, currency, receivedAmount, deposit.user_id);
    ...
    try {
      ...
      await sendTelegramDeposit(
        ...
        `💰 Monto: <b>${receivedAmount} ${currency}</b> (~$${deltaUsd.toFixed(2)} USD)\n` +
        `🌐 Red: <b>${deposit.network ?? "—"}</b>\n` +
        `🆔 ID: <code>${order_id}</code>`,
      );
    } catch (_e) {}

    // 8. Actualizar estadísticas de afiliado si el usuario tiene un referente
    try {
