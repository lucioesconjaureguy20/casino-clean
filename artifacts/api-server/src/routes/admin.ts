...
router.post("/force-confirm-deposit", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const patchR = await sbAdmin(`deposits?id=eq.${deposit_id}&status=eq.pending`, {
      method: "PATCH",
      body: JSON.stringify({ status: "confirmed", amount: deposit.amount, confirmed_at: now }),
...
router.get("/users", async (_req: Request, res: Response) => {
  ...
  try {
    ...
    // ── Fetch balance_demo from Auth metadata ─────────────────────────────────
    ...
    try {
      ...
      if (authUsersRes.ok) {
        ...
        for (const u of authUsers) {
          // Prefer balance_demo_local (current playing balance synced from frontend)
          // over balance_demo (originally credited amount)
...
router.get("/user/:userId/stats", async (req: Request, res: Response) => {
  ...
  try {
    ...
    // All transactions + game bets for this user
    const [txR, wR, depR, balR, betsR] = await Promise.all([
    ...
    const betRows:  any[] = betsR.ok ? await betsR.json() : [];

    // Transactions breakdown
    const bonuses   = txs.filter(t => t.type === "bonus");

    const totalWagered    = betRows.reduce((s: number, b: any) => s + Math.abs(Number(b.bet_usd)), 0);
    const betCount        = betRows.length;
...
router.post("/add-streamer-balance", requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 4. Registrar en transactions como bonus normal (visible en historial del usuario)
