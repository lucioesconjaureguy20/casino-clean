...
import { createSession, getOrCreateSession, clearSession } from "../lib/sessionStore";
...
router.post("/auth/logout", async (req, res) => {
  ...
  // Clear the server-side session (the session_token sent in the custom header)
  ...
  if (authHeader) {
    try {
      ...
      if (r.ok) {
        ...
        if (u?.id) await clearSession(u.id);
      }
    } catch { /* ignore */ }
    try {
  ...
  void clientSessionToken; // acknowledged
...
router.post("/auth/local-token", async (req, res) => {
  ...
  try {
    ...
    // getOrCreateSession: if a session_token already exists in memory (e.g. because
    // the user just logged in via /api/auth/login and this is a background game-token
    // refresh), reuse it instead of creating a new one that would invalidate requests
    // already in-flight with the old token.
    const sessionToken = await getOrCreateSession(profile.id);
