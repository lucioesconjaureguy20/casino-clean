...
async function insertTransaction(
  ...
  const r = await sbAdmin("transactions", {
    ...
    body: JSON.stringify({
      ...
      notes:          `cryptomus:${txHash} deposit_id:${depositId}${usdNote}`,
      completed_at:   new Date().toISOString(),
    }),
  });
  if (!r.ok) console.error("[Cryptomus] error insertando transaction:", await r.text());
  else console.log(`[Cryptomus] tx registrada: ${amount} ${currency} (~$${deltaUsd?.toFixed(2) ?? "?"}) mander=${manderId} display_id=${displayId}`);
}
