...
// Single query fetches all unclaimed pools for the user; filtering is done in memory.
// Per-user 15-second cache to prevent Supabase overload.

const _rakebackCache = new Map<string, { ts: number; data: object }>();
const RAKEBACK_CACHE_MS = 30_000;

router.get("/rakeback-pools", requireAuth, async (req: Request, res: Response) => {
  const userId = req.authUser!.id;
  try {
    const now = Date.now();
    const cached = _rakebackCache.get(userId);
    if (cached && now - cached.ts < RAKEBACK_CACHE_MS) {
    ...
    // Single query — fetch all unclaimed pools for this user, filter in memory
    const r = await sbAdmin(
      `rakeback_pools?user_id=eq.${enc(userId)}&claimed=eq.false&select=pool_type,period_key,amount_usd`,
      { headers: { Prefer: "count=none" } },
    );

    const rows: { pool_type: string; period_key: string; amount_usd: number }[] = r.ok ? await r.json() : [];

    let instant = 0, instant_pending = 0, instant_accum = 0;
    let weekly = 0, weekly_accum = 0, monthly = 0, monthly_accum = 0;

    for (const row of rows) {
      const amt = Number(row.amount_usd);
      const pk  = row.period_key;
      if (row.pool_type === "instant") {
        if (pk >= claimWindowStart && pk < hourBoundary)  instant         += amt;
        else if (pk < claimWindowStart)                    instant_pending += amt;
        else if (pk >= hourBoundary)                       instant_accum   += amt;
      } else if (row.pool_type === "weekly") {
        if (pk < weeklyBoundary)       weekly       += amt;
        else if (pk === weeklyBoundary) weekly_accum += amt;
      } else if (row.pool_type === "monthly") {
        if (pk < monthlyBoundary)        monthly       += amt;
        else if (pk === monthlyBoundary) monthly_accum  += amt;
      }
    }

    const data = {
      instant, instant_pending, instant_accum,
      weekly, weekly_accum,
      monthly, monthly_accum,
      period_keys: {
    ...
    _rakebackCache.set(userId, { ts: now, data });
