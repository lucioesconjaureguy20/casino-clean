...
export async function getOrCreateSession(userId: string): Promise<string> {
  ...
  // Check DB before creating — preserves existing session after server restart.
  // Without this, a post-restart call would generate a new token and overwrite
  // the DB, invalidating any browser that still holds the old token.
  if (await columnExists()) {
    try {
      const r = await sbAdmin(
        `profiles?id=eq.${encodeURIComponent(userId)}&select=session_token&limit=1`,
      ...
      const dbToken = rows?.[0]?.session_token ?? null;
      if (dbToken) {
        activeSessions.set(userId, dbToken);
        return dbToken;
      }
    } catch { /* DB error — fall through to create new */ }
  }
