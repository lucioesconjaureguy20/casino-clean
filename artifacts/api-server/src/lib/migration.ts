...
const IS_DEMO_COL_DDL = `
ALTER TABLE game_bets ADD COLUMN IF NOT EXISTS is_demo BOOLEAN NOT NULL DEFAULT FALSE;
CREATE INDEX IF NOT EXISTS idx_game_bets_is_demo ON game_bets(is_demo);
`;


async function isDemoColExists(): Promise<boolean> {
  try {
    const r = await sbAdmin("game_bets?select=is_demo&limit=0", { headers: { Prefer: "count=none" } });
    return r.ok;
  } catch {
    return false;
  }
...
export async function runMigration(): Promise<void> {
  const [notifExists, rewardExists, isDemoExists] = await Promise.all([
    tableExists("user_notifications"),
    tableExists("user_reward_history"),
    isDemoColExists(),
  ]);

  if (notifExists && rewardExists && isDemoExists) {
    console.log("[migration] Tables OK: user_notifications, user_reward_history, game_bets.is_demo");
    return;
  }
  ...
  if (MANAGEMENT_KEY) {
    const results = await Promise.all([
      ...
      isDemoExists ? Promise.resolve(true) : runSqlViaManagementApi(IS_DEMO_COL_DDL),
    ]);
    if (results.every(Boolean)) {
      console.log("[migration] Tables/columns created via management API.");
      return;
    }
  }
  ...
  if (!isDemoExists) {
    console.warn("[migration] ⚠️  game_bets.is_demo column missing.");
    console.warn("[migration] Run in Supabase SQL editor:");
    console.warn("[migration]   ALTER TABLE game_bets ADD COLUMN IF NOT EXISTS is_demo BOOLEAN NOT NULL DEFAULT FALSE;");
    console.warn("[migration] Or set SUPABASE_MANAGEMENT_KEY for auto-migration.");
  }
}
