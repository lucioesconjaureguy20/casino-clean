...
function RouletteGame({
  ...
  const winPopTimerRef   = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  const ballClearTimer   = useRef<ReturnType<typeof setTimeout> | undefined>(undefined);
  ...
  const spinInsuf   = hasBets && !isSpinning && !isResult && balance < totalBetUsd - 0.0001;
  const canSpin     = hasBets && !isSpinning && !isResult && balance >= totalBetUsd - 0.0001;
  const canSpinBtn  = hasBets && !isSpinning && !isResult; // button is clickable even when insuf (handleSpin calls onInsufficientBalance)
  ...
  function resolveBets(result: number, bets: Record<string, number>, balBefore: number) {
    ...
    // Clear any pending win-popup hide timer from the previous round
    if (winPopTimerRef.current) clearTimeout(winPopTimerRef.current);
    setShowWinPop(false);
    let totalWin = 0;
    ...
    if (totalWin > 0) {
      setShowWinPop(true);
      winPopTimerRef.current = setTimeout(() => setShowWinPop(false), 3000);
    }
    ...
    if (autoRef.current) {
      ...
      if (remaining > 0 || autoInfiniteRef.current) {
        ...
        popTimerRef.current = setTimeout(() => {
          ...
          const wagNext = Object.values(betsForNext).reduce((s, v) => s + v, 0);
          if (newBal < wagNext - 0.0001) { autoRef.current = false; setAutoRunning(false); return; }
  ...
  function handleSpin() {
    ...
    if (hasBets && !isSpinning && !isResult && balance < totalBetUsd - 0.0001) { onInsufficientBalance?.(); return; }
    if (!canSpin) return;
    playBetClick();
  ...
  function handleAutoStart() {
    ...
    if (!hasBets) { autoBtnLockRef.current = false; return; }
    if (balance < totalBetUsd - 0.0001) { autoBtnLockRef.current = false; return; }
    const bets = tableBets;
    setLastBets(bets);
  ...
  return (
    <div className="game-ctrl-flex" style={{ width:"100%", display:"flex", fontFamily:"'Inter',sans-serif", position:"relative", background:"#0e1320", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      <div className="game-ctrl-sidebar" style={{ width:"260px", flexShrink:0, background:"#131a28", borderRight:"1px solid #1a2438", padding:"16px", display: isMobile ? "none" : "flex", flexDirection:"column", gap:"12px" }}>
        ...
        {hasBets && !!currentUser && !isSpinning && !isResult && balance < totalBetUsd - 0.0001 && (
        ...
        {(!isResult || autoRunning || mode === "auto") ? (
          mode === "manual" ? (
            <button onClick={handleSpin} disabled={!canSpinBtn}
              ...
              {isSpinning ? gt(_lang, "rouletteRolling") : (!!currentUser && hasBets && balance < totalBetUsd - 0.0001) ? gt(_lang, "insufficientBal") : gt(_lang, "bjBet")}
            </button>
          ) : (
            (()=>{ const insufAuto = !autoRunning && !!currentUser && hasBets && balance < totalBetUsd - 0.0001; const canStart = !autoRunning && hasBets && !insufAuto; return (
            <button
              ...
              style={{ width:"100%", padding:"14px", borderRadius:"6px", border:"none", fontFamily:"inherit",
                ...
                boxShadow: isSpinning ? "none" : autoStopping ? "0 4px 22px rgba(176,125,32,.4)" : autoRunning ? "0 4px 22px rgba(192,57,43,.4)" : canStart ? "0 4px 22px rgba(26,159,255,.35)" : "none",
                transition:"all .2s" }}
              onMouseEnter={e => {
                if (!isSpinning && !autoStopping && autoRunning) {
                  e.currentTarget.style.transform = "scale(1.03)";
                  e.currentTarget.style.boxShadow = "0 6px 32px rgba(192,57,43,.7)";
                } else if (!isSpinning && !autoStopping && canStart) {
                  e.currentTarget.style.transform = "scale(1.03)";
                  e.currentTarget.style.boxShadow = "0 6px 32px rgba(26,159,255,.65)";
                }
              }}
              onMouseLeave={e => {
                e.currentTarget.style.transform = "";
                e.currentTarget.style.boxShadow = autoStopping ? "0 4px 22px rgba(176,125,32,.4)" : autoRunning
                  ...
                  : canStart ? "0 4px 22px rgba(26,159,255,.35)" : "none";
              }}>
              {autoStopping ? gt(_lang, "bacStopping") : autoRunning ? gt(_lang, "stopAuto") : insufAuto ? gt(_lang, "insufficientBal") : gt(_lang, "startAuto")}
            </button>
            );})()
          )
        ) : (
        ...
        <div className="rlt-icon-footer" style={{ marginTop:"auto", display:"flex", gap:"8px", position:"relative" }}>
      ...
      <div style={{ flex:1, minWidth:0, display:"flex", flexDirection:"column", gap:"16px", padding:"12px 16px 16px", background:"#0e1320", position:"relative" }}>
        ...
        <div style={{
          ...
          <div style={{ position:"relative", width:"245px", height:"245px" }}>
            <canvas ref={canvasRef} width={245} height={245}
              style={{ borderRadius:"50%", boxShadow:"0 0 60px rgba(0,0,0,0.9)", display:"block" }}/>
          ...
          <div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"36px repeat(12, 44px) 48px", gap:"1px" }}>
              ...
              <div
                ...
                style={{
                  gridRow:"1 / span 3",
                  opacity: dragGhost?.fromKey === "n_0" ? 0.35 : 1,
                  position:"relative",
                  background:"#1a6b30",
                  borderRadius:"6px",
                  overflow:"hidden",
                  ...
                  cursor: isSpinning ? "default" : ((tableBets["n_0"]||0) > 0 && dragGhost?.fromKey !== "n_0" ? "grab" : "pointer"),
                  userSelect:"none",
                  border: winCells.has("n_0") ? "2px solid #16ff5c" : "2px solid rgba(255,255,255,0.08)",
                  boxShadow: winCells.has("n_0") ? "0 0 10px rgba(22,255,92,0.5)" : "none",
                ...
                onMouseEnter={e => { if (!isSpinning && !isDragging) { const el = e.currentTarget as HTMLElement; el.style.filter="brightness(1.3)"; el.style.zIndex="5"; }}}
                onMouseLeave={e => { const el = e.currentTarget as HTMLElement; el.style.filter=""; el.style.zIndex=""; }}>
                {(tableBets["n_0"]||0) > 0 && dragGhost?.fromKey !== "n_0" ? (
                  <div
                    style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", zIndex:3, cursor:"grab", touchAction:"none", WebkitTapHighlightColor:"transparent" }}
                    onClick={e => e.stopPropagation()}
              ...
              return (
                <>
                  ...
                  <div style={{ position:"absolute", top:0, left:0, width:"100%", height:"134px", pointerEvents:"none", overflow:"visible" }}>
                    ...
                    {/* Zero boundary: splits 0-3, 0-2, 0-1 + trios 0-2-3 and 0-1-2 */}
                    <RZone {...zp} zkey="sp_0_3" title="Semipleno: 0-3 (17:1)" style={{ left:30, top:15,   width:14, height:14, borderRadius:"50%" }} />
                    <RZone {...zp} zkey="sp_0_2" title="Semipleno: 0-2 (17:1)" style={{ left:30, top:60,   width:14, height:14, borderRadius:"50%" }} />
                    <RZone {...zp} zkey="sp_0_1" title="Semipleno: 0-1 (17:1)" style={{ left:30, top:105,  width:14, height:14, borderRadius:"50%" }} />
                    <RZone {...zp} zkey="tr_0_2_3" title="Trío: 0-2-3 (11:1)"  style={{ left:30, top:37,   width:14, height:14, borderRadius:"50%", zIndex:9 }} />
                    <RZone {...zp} zkey="tr_0_1_2" title="Trío: 0-1-2 (11:1)"  style={{ left:30, top:82,   width:14, height:14, borderRadius:"50%", zIndex:9 }} />
                  </div>
        ...
        {isMobile && (
          <div style={{ padding:"8px 6px 10px", display:"flex", flexDirection:"column", gap:"8px", opacity: isSpinning ? 0.55 : 1, transition:"opacity .2s", pointerEvents: isSpinning ? "none" : "auto" }}>
              ...
              (isResult && mobileWheelLinger) ? (
                ...
                <button onClick={handleSpin} disabled={!canSpin}
                  style={{ width:"100%", padding:"14px", borderRadius:"8px", border:"none", fontFamily:"inherit",
                    ...
                    boxShadow: canSpin ? "0 4px 22px rgba(26,159,255,.35)" : "none",
                    transition:"all .2s" }}>
                  {(!!currentUser && hasBets && balance < totalBetUsd-0.0001) ? gt(_lang,"insufficientBal") : gt(_lang,"bjBet")}
                </button>
              ) : (
                <button onClick={handleNewRoundAndSpin}
                  style={{ width:"100%", padding:"14px", borderRadius:"8px", border:"none", fontFamily:"inherit",
            ...
            {/* 7 — Stats / Volume icon buttons (mobile) */}
            <div className="rlt-icon-footer" style={{ display:"flex", gap:"8px", alignItems:"center", paddingTop:"4px" }}>
              <button
