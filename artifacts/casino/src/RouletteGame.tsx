...
export default function RouletteGame({
  ...
  const canSpin     = hasBets && !isSpinning && !isResult;
                    ...
                    return (
                      <button key={v} onClick={() => setChipUsd(v)}
                        ...
                        onMouseEnter={e => {
                          ...
                          if (!sel) e.currentTarget.style.filter = "drop-shadow(0 0 9px rgba(255,210,60,0.75)) drop-shadow(0 0 4px rgba(255,255,255,0.5))";
                        }}
                        onMouseLeave={e => {
                          e.currentTarget.style.transform = "scale(1)";
                          e.currentTarget.style.filter = sel
                            ? "drop-shadow(0 0 7px rgba(255,220,100,0.85)) drop-shadow(0 0 3px rgba(255,255,255,0.6))"
                            : "drop-shadow(0 2px 3px rgba(0,0,0,0.5))";
                        }}>
        ...
        {(!isResult || autoRunning || mode === "auto") ? (
          mode === "manual" ? (
            ...
            <button
              ...
              disabled={isSpinning}
              style={{ width:"100%", padding:"14px", borderRadius:"12px", border:"none", fontFamily:"inherit",
                background: autoRunning ? "linear-gradient(135deg,#c0392b,#a93226)" : (hasBets || Object.keys(lastBets).length > 0) ? "linear-gradient(180deg,#1a9fff,#0d6fd4)" : "#1a2438",
                color: autoRunning || (hasBets || Object.keys(lastBets).length > 0) ? "#fff" : "#3a4a60",
                fontWeight:800, fontSize:"14px",
                cursor: isSpinning ? "not-allowed" : "pointer",
                boxShadow: autoRunning ? "0 4px 22px rgba(192,57,43,.4)" : (hasBets || Object.keys(lastBets).length > 0) ? "0 4px 22px rgba(26,159,255,.35)" : "none",
                transition:"all .2s" }}
              onMouseEnter={e => {
                if (!isSpinning) {
