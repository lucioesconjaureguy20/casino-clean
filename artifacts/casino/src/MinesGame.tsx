...
// Preload images at module level — fires when the JS bundle is first imported
// (on app load), not on component mount. This ensures images are already in
// the browser cache before the user navigates to Mines.
if (typeof window !== "undefined") {
  ["/diamond.webp", "/bomb2.webp"].forEach(path => {
    const img = new Image();
...
function GemIcon({ size = 36, dim = false }: { size?: number; dim?: boolean }) {
  return (
    <img
      src={assetUrl("/diamond.webp")}
      alt="gem"
      width={size}
      height={size}
      style={{
        objectFit:"contain", opacity: dim ? 0.3 : 1, display:"block", pointerEvents:"none",
        animation: dim ? undefined : "minesGemEnter 0.35s cubic-bezier(0.22,1,0.36,1) both, minesGemShimmer 2.8s ease-in-out 0.4s infinite",
...
function MineIcon({ size = 34, exploding = false }: { size?: number; exploding?: boolean }) {
  // Track whether the explosion animation has already played so we don't
  // re-run the entrance animation after the explosion settles.
  const hasExplodedRef = useRef(false);
  useEffect(() => {
    if (exploding) hasExplodedRef.current = true;
  }, [exploding]);

  let anim: string | undefined;
  if (exploding) {
    anim = "bombExplode 0.55s cubic-bezier(0.36,0.07,0.19,0.97) both";
  } else if (!hasExplodedRef.current) {
    // First reveal with no explosion: pop in instantly with a quick scale bounce
    anim = "minesBombEnter 0.32s cubic-bezier(0.22,1,0.36,1) both";
  }
  // After explosion settles (hasExplodedRef = true, exploding = false): no animation

  return (
    <img
      src={assetUrl("/bomb2.webp")}
      alt="bomb"
      width={size}
      height={size}
      style={{
...
export default function MinesGame({
  ...
  onGameActive, lang, displayInFiat = false, onInsufficientBalance,
}: MinesGameProps) {
  ...
  const [minesStatsCollapsed, setMinesStatsCollapsed] = useState(false);
  const [minesStatsScale, setMinesStatsScale] = useState(1);
  ...
  const minesStatsPinchDist = useRef<number|null>(null);
  const minesStatsPinchScale = useRef(1);
  const minesChartRaf = useRef<number | null>(null);
  ...
  function handleMinesStatsDragStart(e: React.MouseEvent) {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      rafId = requestAnimationFrame(() => {
        setMinesStatsPos({ x: cx - minesStatsDragOffset.current.x, y: cy - minesStatsDragOffset.current.y });
        rafId = null;
      });
    };
    const onUp = () => {
  ...
  function placeBet() {
    if (!currentUser) { onRequestLogin?.(); return; }
    if (!!currentUser && balInsuff) { onInsufficientBalance?.(); return; }
  ...
  // ── Pick a random hidden cell and reveal it ───────────────────────────
  function pickRandomCell() {
    if (phase !== "playing") return;
    const hidden = gridRef.current
      .map((c, i) => ({ c, i }))
  ...
  function cellStyle(cell: Cell, idx: number): React.CSSProperties {
    ...
    return {
      ...
      borderRadius: gridSize >= 7 ? "6px" : "8px",
      border, background: bg, boxShadow,
      display: "flex", alignItems: "center", justifyContent: "center",
      cursor: isClickable ? "pointer" : "default",
      transition: isHidden ? "background 0.15s ease, border 0.15s ease, transform 0.15s ease" : "none",
      position: "relative",
      overflow: isExploding ? "visible" : "hidden",
      animation: isExploding ? "minesExplodeCell 0.55s cubic-bezier(0.36,0.07,0.19,0.97) both"
               : undefined,
    };
  }
  ...
  return (
    <div className="mines-root" style={{ display:"flex", flexDirection:"column", width:"100%", height:"100%", background:"#0e1320", overflow: isMobile ? "visible" : "hidden", position:"relative", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      <style>{`
        ...
        @keyframes minesGemEnter {
          0%   { transform: scale(0.3) translateY(10px); opacity: 1; filter: brightness(2.0) drop-shadow(0 0 6px rgba(80,180,255,0.5)); }
          65%  { transform: scale(1.15) translateY(-3px); filter: brightness(1.15) drop-shadow(0 0 8px rgba(80,180,255,0.7)); }
          82%  { transform: scale(0.95) translateY(1px); }
          100% { transform: scale(1) translateY(0); opacity: 1; filter: drop-shadow(0 0 6px rgba(80,180,255,0.5)); }
        }
        @keyframes minesBombEnter {
          0%   { transform: scale(0.3) translateY(8px); filter: brightness(1.5); }
      ...
      <div className="game-ctrl-sidebar" style={{ width:"260px", flexShrink:0, background:"#131a28", borderRight:"1px solid #1a2438", display:"flex", flexDirection:"column", padding:0, gap:0, overflow:"hidden" }}>
        ...
        {/* ── BET / CASHOUT BUTTON — mobile order 1 ── */}
        {!autoMode && (
          <div style={{ order: isMobile ? 1 : 50, marginBottom: isMobile ? "10px" : 0 }}>
            {phase !== "playing" && (
              <button onClick={placeBet} disabled={betInvalid}
                style={{
            ...
            {(["Manual","Auto"] as const).map(tab=>{
              ...
              return (
                <button key={tab} onClick={()=>{
                  if(autoRunning || phase==="playing") return;
        ...
        <div style={{ order: isMobile ? 2 : 1 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:"8px", background:"#0e1826", border:`1px solid ${betInvalid||balInsuff?"#e74c3c":"#252f45"}`, borderRadius:"6px", padding:"8px 14px", marginBottom:"6px", transition:"border .15s" }}>
            ...
            <input
              ...
              onBlur={() => {
                ...
                const c = maxBetDisplay <= 0 ? 0 : Math.max(minBetDisplay, Math.min(maxBetDisplay, raw || minBetDisplay));
          ...
          {(betInvalid || balInsuff) ? (
            <div style={{ fontSize:"11px", color:"#e74c3c", fontWeight:600, marginBottom:"5px", paddingLeft:"2px" }}>
              ...
              {betInvalid && <div>Mín: {fmtMoney(0.01)}</div>}
            </div>
          ) : <div style={{ marginBottom:"5px" }} />}
        </div>

        {/* Quick bet — mobile order 3 */}
        <div style={{ order: isMobile ? 3 : 2, marginBottom:"12px" }}>
        ...
        {/* Mines selector — dual-color slider */}
        {(()=>{
          const maxMines = maxMinesFor(gridSize);
          const totalCells = gridSize * gridSize;
              ...
              <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                ...
                <div style={{ flex:"1 1 0", minWidth:0 }}>
                  <input
                    ...
                    onChange={e=>{ if(!disabled&&!autoSelecting){ resetAutoIfNeeded(); setMineCount(totalCells - Number(e.target.value)); } }}
                  />
                </div>
                {/* Right badge — mines */}
                <div style={{ display:"flex", alignItems:"center", gap:"5px", background:"#200d0d", border:"1px solid #4a1010", borderRadius:"6px", padding:"5px 8px", width:"58px", justifyContent:"center", flexShrink:0, flexGrow:0 }}>
                  <span style={{ color:"#e84a4a", fontWeight:700, fontSize:"14px", lineHeight:1 }}>{mineCount}</span>
                  <img src={assetUrl("/bomb2.webp")} width={16} height={16} style={{ objectFit:"contain" }} alt="mine"/>
                </div>
              </div>
            </div>
          );
        ...
        {/* Board size — mobile: collapsible dropdown (order 5); desktop: grid buttons */}
        <div style={{ order: isMobile ? 5 : 4, marginBottom:"12px" }}>
          {isMobile ? (
        ...
        {/* ── AUTO: Start Auto / Select Cells — mobile only, order 1 (first) ── */}
        {autoMode && isMobile && (<div style={{ order: 1, marginBottom:"12px" }}>
          {!autoRunning ? (
            ...
            <button onClick={()=>stopAutoMines()}
              style={{ width:"100%", border:"none", borderRadius:"6px", padding:"14px", fontWeight:700, fontSize:"15px", background:"linear-gradient(180deg,#ef4444,#b91c1c)", color:"#fff", cursor:"pointer", boxShadow:"0 4px 22px rgba(239,68,68,0.35)" }}>
              {T("minesStopAuto")}
            </button>
          )}
        </div>)}

        {/* ── AUTO MODE CONFIG — mobile order 6 ── */}
        {autoMode && (<div style={{ order: isMobile ? 6 : 50 }}><>
        ...
        {/* ── AUTO MODE config details (cashout/bets/advanced + desktop start btn) — mobile order 7 ── */}
        {autoMode && (<div style={{ order: isMobile ? 7 : 50 }}><>
          ...
          <div style={{ marginBottom:"12px" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"6px", background:"#0e1826", border:"1px solid #252f45", borderRadius:"6px", padding:"6px 10px" }}>
              <input
                ...
                onWheel={e=>{ if (!autoInfinite) e.currentTarget.blur(); }}
          ...
          {/* Iniciar Auto / Detener Auto — desktop only */}
          {!isMobile && (!autoRunning ? (
            <button onClick={startAutoMines} disabled={autoSelectedCells.size===0||betInvalid||(!!currentUser&&balInsuff)}
              style={{
                width:"100%", border:"none", borderRadius:"6px", padding:"14px",
                fontWeight:700, fontSize:"15px", transition:"all .2s",
        ...
        <div style={{ flexShrink:0, padding:"8px 16px 16px 16px", display:"flex", gap:"8px", position:"relative", zIndex:1, borderTop: autoMode ? "1px solid #1e2840" : "none" }}>
          ...
          <div role="button" tabIndex={0} onClick={() => setMinesVol(v => v > 0 ? 0 : 70)}
      ...
      <div className="mines-main-area" style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding: isMobile ? "14px 8px" : "20px 28px", position:"relative", background:"#0e1320", overflow:"hidden", boxSizing:"border-box", width:"100%" }}>
        ...
        {(() => {
          const gridPx = isMobile ? Math.min(window.innerWidth - 64, 390) : 580;
          return (
        <div className="mines-grid-outer" style={{ position:"relative", width:`${gridPx}px`, height:`${gridPx}px`, flexShrink:0 }}>
          <div style={{
            display: "grid",
            ...
            {displayGrid.map((cell, idx) => (
              <div key={idx}
                ...
                onClick={e=>{ (e.currentTarget as HTMLDivElement).style.transform = ""; if(autoMode) { if(!autoRunning && phase==="idle") toggleAutoCell(idx); } else revealCell(idx); }}
          ...
          {resultOverlay && resultOverlay.type === "win" && (
            <div onClick={dismissOverlay} style={{
              ...
              <div style={{
                ...
                borderRadius:"12px", padding:"28px 44px", textAlign:"center",
                ...
                <div style={{ position:"relative",background:"#0a1520",borderRadius:"6px",height:"190px",overflow:"visible",border:"1px solid #1a3347" }}>
                  ...
                  <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ width:"100%",height:"100%",display:"block",cursor:"crosshair" }}
                    onMouseMove={e=>{
                      ...
                      if(minesChartRaf.current!==null) cancelAnimationFrame(minesChartRaf.current);
                      minesChartRaf.current=requestAnimationFrame(()=>{
                        ...
                        minesChartRaf.current=null;
                      });
                    }}
                    onMouseLeave={()=>{ if(minesChartRaf.current!==null){cancelAnimationFrame(minesChartRaf.current);minesChartRaf.current=null;} setMinesChartHover(null); }}
                  >
                    <defs>
