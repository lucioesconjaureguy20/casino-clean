...
export default function MinesGame({
  ...
  onGameActive, lang, displayInFiat = false,
}: MinesGameProps) {
  ...
  return (
    <div className="mines-root" style={{ display:"flex", flexDirection:"column", width:"100%", height:"100%", background:"#0e1320", overflow: isMobile ? "visible" : "hidden", position:"relative", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      {/* ── BODY ─────────────────────────────────────────────────────────── */}
      <div className="game-ctrl-flex" style={{ display:"flex", flex:1, overflow: isMobile ? "visible" : "hidden" }}>
      ...
      <div className="game-ctrl-sidebar" style={{ width:"260px", flexShrink:0, background:"#131a28", borderRight:"1px solid #1a2438", display:"flex", flexDirection:"column", padding:0, gap:0, overflow:"hidden" }}>
          ...
          {(["Manual","Auto"] as const).map(tab=>{
            ...
            return (
              <button key={tab} onClick={()=>{
                ...
                style={{ flex:1, background:active?"#1e2c44":"transparent", border:active?"1px solid #3a4a60":"1px solid transparent", borderRadius:"6px", padding:"10px", color:active?"#eef3f8":"#5a6a88", fontWeight:500, fontSize:"14px", cursor:autoRunning?"not-allowed":"pointer", opacity:autoRunning&&!active?0.45:1, transition:"opacity .2s" }}>
                {tabLabel}
              </button>
            );
          })}
        ...
        {!autoMode && (<>
          ...
          {phase === "playing" && revealedSafe > 0 && (
            <button onClick={()=>cashout()}
              style={{ width:"100%", padding:"14px", borderRadius:"6px", border:"none", background:"linear-gradient(180deg,#f4a91f,#d4890f)", color:"#000", fontWeight:700, fontSize:"15px", cursor:"pointer", boxShadow:"0 4px 20px rgba(244,169,31,0.4)", animation:"minesPulse 2s ease infinite", marginBottom:"12px" }}>
              {T("minesCobrar")} {fmtMoney(cashoutAmount)}
            </button>
          )}
          {phase === "playing" && revealedSafe === 0 && (
            <div style={{ width:"100%", padding:"12px", borderRadius:"6px", background:"#0e1826", border:"1px solid #1a2438", color:"#5a6a88", fontWeight:600, fontSize:"13px", textAlign:"center", marginBottom:"12px" }}>
              {T("minesClickCell")}
            </div>
          )}
          {phase === "playing" && (
            <div style={{ background:"#0a1420", border:"1px solid #1a2a40", borderRadius:"6px", padding:"10px 14px", animation:"minesFadeIn 0.25s ease" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px", marginBottom:"12px" }}>
            ...
            <div style={{ minWidth:0 }}>
              ...
              <div style={{ display:"flex", alignItems:"center", background:"#0e1826", border:"1px solid #252f45", borderRadius:"8px", padding:"0 6px 0 8px", height:"32px", gap:"3px" }}>
                <input
                  value={autoInfinite ? T("infiniteLabel") : autoNumBetsInput}
        ...
        <div style={{ flexShrink:0,padding:"8px 16px 16px 16px",display:"flex",gap:"8px",position:"relative" }}>
          ...
          <div style={{ position:"relative" }}>
            ...
            {showMinesVol && (
              <div style={{ position:"absolute",bottom:"48px",left:"0",background:"#0f1e2e",border:"1px solid #252f45",borderRadius:"6px",padding:"10px 16px",display:"flex",alignItems:"center",gap:"12px",minWidth:"220px",boxShadow:"0 4px 20px rgba(0,0,0,.5)",zIndex:10000 }}>
