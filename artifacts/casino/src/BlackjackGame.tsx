...
type BJPhase = "idle" | "player" | "dealer" | "result" | "insurance";
...
export default function BlackjackGame({
  ...
  const deal = useCallback(() => {
    ...
    if (phase === "player" || phase === "dealer" || dealingRef.current) return;
    if (betUsd > localBalRef.current + 0.0001 || betUsd < 0.0099) return;

    // Clear any persisted hand so a fresh game starts clean
    localStorage.removeItem("bj_game_state");
  ...
  const disabled  = phase === "player" || phase === "dealer" || phase === "insurance";
    ...
    <div style={{ display:"flex", height:"100%", overflow:"hidden", background:"#0e1320", flexDirection:"column", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <div style={{ width:"268px", flexShrink:0, background:"#131a28", borderRight:"1px solid #1a2438", display:"flex", flexDirection:"column", gap:0, padding:"16px" }}>
          ...
          {/* Deal / Apostar button */}
          {(() => {
            const dealDisabled = phase === "player" || phase === "dealer" || phase === "insurance" || betInvalid || (!!currentUser && balInsuff);
