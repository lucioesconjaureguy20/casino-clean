...
// ── Suit SVG (never renders as emoji on any platform) ─────────────────────────
function SuitSVG({ suit, size = 14, color }: { suit: string; size?: number; color: string }) {
  // All paths fit in viewBox "0 0 12 12"
  const paths: Record<string, string> = {
    "♥": "M6,11 C2,8 0.5,6 0.5,4 C0.5,2.2 1.8,1 3.5,1 C4.4,1 5.2,1.6 6,2.8 C6.8,1.6 7.6,1 8.5,1 C10.2,1 11.5,2.2 11.5,4 C11.5,6 10,8 6,11 Z",
    "♦": "M6,0.5 L11.5,6 L6,11.5 L0.5,6 Z",
    "♠": "M6,1 L11,7 C9,9.5 7.2,8.5 7.2,8.5 L7.8,11 L4.2,11 L4.8,8.5 C4.8,8.5 3,9.5 1,7 Z",
    "♣": "M6,11 L5.1,11.8 L6.9,11.8 Z M5.5,7.8 L5.5,11.1 L6.5,11.1 L6.5,7.8 Z M6,1 A2.2,2.2 0 0 0 3.8,3.2 A2.2,2.2 0 1 0 5.3,7.6 A2.2,2.2 0 0 0 6,8.5 A2.2,2.2 0 0 0 6.7,7.6 A2.2,2.2 0 1 0 8.2,3.2 A2.2,2.2 0 0 0 6,1 Z",
  };
  return (
      ...
      {showHidden ? (
        ...
        <>
          <span style={{
            ...
            <span>{card.rank}</span>
            <SuitSVG suit={card.suit} size={small ? 11 : 13} color={red ? "#cc1a1a" : "#111"} />
          </span>
          <span style={{ display:"flex", alignItems:"center", justifyContent:"center" }}>
            <SuitSVG suit={card.suit} size={small ? 26 : 34} color={red ? "#cc1a1a" : "#111"} />
          </span>
          <span style={{
...
export default function BlackjackGame({
  ...
  currentUser, onRequestLogin, onGameActive, displayInFiat = false,
}: BlackjackGameProps) {
  ...
  return (
    <div style={{ display:"flex", height:"100%", overflow:"hidden", background:"#0e1320", flexDirection:"column", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      {/* ── Body ──────────────────────────────────────────────────────────── */}
      <div className="game-ctrl-flex" style={{ display:"flex", flex:1, overflow:"hidden" }}>

        {/* ── Left panel ─────────────────────────────────────────────────── */}
        <div className="game-ctrl-sidebar" style={{ width:"268px", flexShrink:0, background:"#131a28", borderRight:"1px solid #1a2438", display:"flex", flexDirection:"column", gap:0, padding:"16px" }}>
          ...
          {(() => {
            ...
            return (
              <button onClick={deal} disabled={dealDisabled} className="bj-deal-btn"
                style={{ width:"100%", marginBottom:"14px", padding:"14px", borderRadius:"6px", fontWeight:500, fontSize:"15px", border:"none",
          ...
          <div style={{ marginTop:"auto", display:"flex", gap:"8px", position:"relative" }}>
            ...
            <div style={{ position:"relative" }}>
              ...
              {showVol && (
                <div style={{ position:"absolute",bottom:"48px",left:"0",background:"#0f1e2e",border:"1px solid #252f45",borderRadius:"6px",padding:"10px 16px",display:"flex",alignItems:"center",gap:"12px",minWidth:"200px",boxShadow:"0 4px 20px rgba(0,0,0,.5)",zIndex:10000 }}>
        ...
        {/* ── Table area ─────────────────────────────────────────────────── */}
        <div ref={tableAreaRef} style={{ flex:1, position:"relative", display:"flex", flexDirection:"column", justifyContent:"space-between", overflow:"hidden",
          background:"#0e1320" }}>
          ...
          {/* ── Dealer hand ── */}
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"10px", paddingTop: isMobile ? "16px" : "32px", position:"relative", zIndex:2 }}>
            <div style={{ fontSize:"11px", fontWeight:500, color:"rgba(255,255,255,.45)", letterSpacing:"2px" }}>{T("bjDealer")}</div>
            <div style={{ display:"flex", gap:"10px", justifyContent:"center", minHeight: isMobile ? "80px" : "120px", alignItems:"center", flexWrap:"wrap" }}>
              ...
              {dealerHand.length === 0 && (
                <div style={{ width: isMobile ? "65px" : "82px", height: isMobile ? "96px" : "118px", borderRadius:"10px", border:"2px dashed rgba(255,255,255,.1)", opacity:.5 }} />
              )}
            </div>
          ...
          {/* ── Player hands ── */}
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"10px", paddingBottom: isMobile ? "12px" : "28px", position:"relative", zIndex:2 }}>
            ...
            {isSplit ? (
              ...
              <>
                ...
                <div style={{ display:"flex", gap:"10px", justifyContent:"center", alignItems:"flex-end", minHeight: isMobile ? "70px" : "120px", flexWrap:"wrap" }}>
                  {playerHand.length > 0 ? (
                    playerHand.map((card, i) => {
                      const key = `${animKey}-p${i}`;
                      const { fx, fy } = cardOff(key);
