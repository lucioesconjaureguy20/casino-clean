...
// ── Card component (same style as HiLo for consistency) ──────────────────────
function BacCard({ card, delay = 0, glow = false, small = false }: {
  ...
  const rankFs   = small ? "12px" : "18px";
  const suitSmFs = small ? "10px" : "14px";
  const centerFs = small ? "26px" : "44px";
  const color    = red ? "#cc1a1a" : "#111";
  const pad      = small ? "5px 6px" : "7px 9px";
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          userSelect:"none", padding: pad,
        }}>
          {/* Top-left */}
          <span style={{
            position:"absolute", top: small ? "4px" : "6px", left: small ? "5px" : "8px",
            fontSize:rankFs, fontWeight:900, color, lineHeight:1.1,
          ...
          {/* Center suit */}
          <span style={{ fontSize:centerFs, color, lineHeight:1 }}>{card.suit}</span>
          {/* Bottom-right */}
          <span style={{
            position:"absolute", bottom: small ? "4px" : "6px", right: small ? "5px" : "8px",
            fontSize:rankFs, fontWeight:900, color, lineHeight:1.1,
            display:"flex", flexDirection:"column", alignItems:"center",
...
export default function BaccaratGame({
  ...
  stopAutoRef, onGameActive, lang, onInsufficientBalance,
}: BaccaratGameProps) {
  ...
  function handleDeal() {
    ...
    if (!!currentUser && hasBets && balance < totalBet) { onInsufficientBalance?.(); return; }
    dealRound(zoneBets);
  }
  ...
  const canDealBtn = (phase === "idle" && hasBets) || phase === "result"; // clickable even when balance insuf
    ...
    <div className="game-ctrl-grid baccarat-root" style={{ display:"grid", gridTemplateColumns:"300px 1fr", background:"#0e1320", height:"100%", flex:1, fontFamily:"'Inter',sans-serif", userSelect:"none", WebkitUserSelect:"none" }}>
      ...
      <div className="game-ctrl-panel" style={{ background:"#131a28", borderRight:"1px solid #1a2438", display:"flex", flexDirection:"column", overflowY:"auto" }}>
        ...
        <div style={{ padding:"16px 14px", flex:1, display:"flex", flexDirection:"column", gap:"14px" }}>
          ...
          {/* 1. Deal / Auto button — TOP */}
          <div className="bac-deal-btn-wrap" style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
          {tab === "manual" ? (
            <>
              {phase === "idle" && !!currentUser && hasBets && balance < totalBet && (
              ...
              <button onClick={handleDeal} disabled={!canDealBtn}
                ...
                {phase === "dealing" ? (
                  <span style={{display:"flex",alignItems:"center",gap:"6px",justifyContent:"center"}}>
                    <img loading="lazy" decoding="async" src={assetUrl(`${import.meta.env.BASE_URL}emoji-repartiendo.webp`)} style={{width:20,height:20,objectFit:"contain",flexShrink:0,filter:"drop-shadow(0 0 4px rgba(26,159,255,.5))"}}/>
                    {gt(lang,"bacDealing")}
                  </span>
                ) : gt(lang,"bjBet")}
              </button>
            ...
            <>
              {(()=>{
                const isDealing = phase === "dealing";
                const insufAuto = !autoRunning && hasBets && !!currentUser && balance < totalBet;
                ...
                return (<>
                  ...
                  <button onClick={handleAutoButton} disabled={btnDisabled}
                    ...
                    {gt(lang, autoStopping ? "bacStopping" : autoRunning ? "stopAuto" : insufAuto ? "insufficientBal" : "startAuto")}
                  </button>
                </>);
              })()}
            </>
          ...
          <div className="bac-icon-footer" style={{ marginTop:"auto", display:"flex", gap:"8px", position:"relative" }}>
