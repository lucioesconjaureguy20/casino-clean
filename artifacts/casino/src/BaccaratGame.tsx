...
export default function BaccaratGame({
  ...
  async function dealRound(zb: Record<BetZone, number>) {
    ...
    pHand.push(p1); setPlayerCards([...pHand]); playDealSound(sv); setTimeout(() => playFlipSound(sv), 85); await wait(340);
    bHand.push(b1); setBankerCards([...bHand]); playDealSound(sv); setTimeout(() => playFlipSound(sv), 85); await wait(340);
    pHand.push(p2); setPlayerCards([...pHand]); playDealSound(sv); setTimeout(() => playFlipSound(sv), 85); await wait(340);
    bHand.push(b2); setBankerCards([...bHand]); playDealSound(sv); setTimeout(() => playFlipSound(sv), 85); await wait(400);
    ...
    if (pScore0 < 8 && bScore0 < 8) {
      if (needsPlayerThird(pScore0)) {
        p3 = d[idx++];
        pHand.push(p3);
        setPlayerCards([...pHand]);
        playDealSound(sv); setTimeout(() => playFlipSound(sv), 85);
        await wait(380);
      }
      if (needsBankerThird(bScore0, pScore0, p3)) {
        b3 = d[idx++];
        bHand.push(b3);
    ...
    // Return push bets on player side on tie
    if (winner === "tie" && zb.player > 0) winAmt += zb.player;

    // True win: the player has a bet on the winning zone
    const has_win =
      (winner === "player" && zb.player > 0) ||
      (winner === "banker" && zb.banker > 0) ||
      (winner === "tie"    && zb.tie    > 0);
    const is_push = winner === "tie" && !has_win; // banker/player bets returned, no tie bet

    // Multiplier for the winning zone (for the win popup)
            ...
            <>
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"6px", background:"#0e1826", border:"1px solid #252f45", borderRadius:"10px", padding:"6px 10px" }}>
                  <input
                    value={autoRunning ? String(autoRemaining) : (autoInfinite ? "∞" : autoCount)}
              ...
              <button onClick={autoRunning ? stopAuto : startAuto}
                disabled={!autoRunning && (!hasBets || balance < totalBet)}
                style={{ width:"100%", padding:"14px", borderRadius:"12px", border:"none", fontFamily:"inherit",
                  background: autoRunning ? "linear-gradient(135deg,#c0392b,#a93226)" : (hasBets ? "linear-gradient(180deg,#1a9fff,#0d6fd4)" : "#1a2438"),
                  color: autoRunning || hasBets ? "#fff" : "#3a4a60",
                  fontWeight:800, fontSize:"14px", cursor:"pointer", transition:"all .2s" }}>
                {autoRunning ? "⏹ Detener Auto" : "▶ Iniciar Auto"}
              </button>
            </>
          )}
