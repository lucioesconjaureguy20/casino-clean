...
// ── Card component ────────────────────────────────────────────────────────────
function BacCard({ card, delay = 0, glow = false, small = false }: {
  card: Card; delay?: number; glow?: boolean; small?: boolean;
}) {
  ...
  const cw = small ? 72 : 110;
  const ch = small ? 100 : 158;

  return (
    <div style={{
      width:`${cw}px`, height:`${ch}px`, borderRadius:"10px", flexShrink:0,
      ...
      <div style={{
        ...
        <div style={{
          ...
          display:"flex", flexDirection:"column", padding: small ? "5px 6px" : "7px 9px",
          justifyContent:"space-between",
        }}>
          <div style={{ fontSize: small ? "13px" : "19px", fontWeight:800, color: red ? "#d42020" : "#111", lineHeight:1, display:"flex", alignItems:"center", gap:"2px" }}>
            {card.rank}<SuitSVG suit={card.suit} size={small ? 10 : 14} color={red ? "#d42020" : "#111"} />
          </div>
          <div style={{ textAlign:"center", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <SuitSVG suit={card.suit} size={small ? 28 : 42} color={red ? "#d42020" : "#111"} />
          </div>
          <div style={{ fontSize: small ? "13px" : "19px", fontWeight:800, color: red ? "#d42020" : "#111", transform:"rotate(180deg)", lineHeight:1, display:"flex", alignItems:"center", gap:"2px" }}>
            {card.rank}<SuitSVG suit={card.suit} size={small ? 10 : 14} color={red ? "#d42020" : "#111"} />
          </div>
        </div>
        <div style={{
          ...
          <img src="/mander-logo.png" alt="" style={{ width: small ? "28px" : "40px", opacity:0.5, position:"relative", zIndex:1 }} />
        </div>
      </div>
    </div>
  );
...
export default function BaccaratGame({
  ...
  const bacChartRaf = useRef<number | null>(null);
  ...
  function handleStatsDragStart(e: React.MouseEvent) {
    ...
    const onUp = () => { if (rafId !== null) { cancelAnimationFrame(rafId); rafId = null; } isStatsDragging.current = false; window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", onUp); };
  ...
  return (
    <div className="game-ctrl-grid" style={{ display:"grid", gridTemplateColumns:"300px 1fr", background:"#0e1320", height:"100%", flex:1, fontFamily:"'Inter',sans-serif", userSelect:"none", WebkitUserSelect:"none" }}>
                    ...
                    <div style={{ position:"relative", background:"#0a1520", borderRadius:"6px", height:"160px", border:"1px solid #1a3347", overflow:"visible" }}>
                      ...
                      <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none"
                        ...
                        onMouseMove={e => {
                          ...
                          if(bacChartRaf.current!==null) cancelAnimationFrame(bacChartRaf.current);
                          bacChartRaf.current=requestAnimationFrame(()=>{
                            ...
                            bacChartRaf.current=null;
                          });
                        }}
                        onMouseLeave={()=>{ if(bacChartRaf.current!==null){cancelAnimationFrame(bacChartRaf.current);bacChartRaf.current=null;} setChartHover(null); }}>
                        <defs>
      ...
      <div style={{ display:"flex", flexDirection:"column", background:"#0e1320", position:"relative", overflow:"hidden" }}>
        ...
        {/* ── CARD DISPLAY ─────────────────────────────────────────────── */}
        <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding: isMobile ? "20px 10px 12px" : "50px 32px 16px" }}>
          <div style={{ display:"flex", alignItems:"flex-start", gap: isMobile ? "8px" : "24px", width:"100%", maxWidth:"780px" }}>
            ...
            <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:"12px" }}>
              ...
              {(() => {
                ...
                const FO = isMobile ? 18 : 44, CW = isMobile ? 72 : 110, CH = isMobile ? 100 : 158;
                  ...
                  <div style={{ position:"relative", width:fw, height:CH, flexShrink:0 }}>
                    {playerCards.map((c,i) => (
                      <div key={i} style={{ position:"absolute", left:i*FO, top:0, zIndex:i+1 }}>
                        <BacCard card={c} delay={0} glow={result==="player"} small={isMobile} />
                      </div>
                    ))}
                    {n===0 && phase==="idle" && (
              ...
              {(() => {
                ...
                return (
                  <div style={{ position:"relative", width:fw, height:CH, flexShrink:0 }}>
                    {bankerCards.map((c,i) => (
                      <div key={i} style={{ position:"absolute", left:i*FO, top:0, zIndex:i+1 }}>
                        <BacCard card={c} delay={0} glow={result==="banker"} small={isMobile} />
                      </div>
                    ))}
                    {n===0 && phase==="idle" && (
                      <div style={{ width:CW, height:CH, borderRadius:"6px", border:"2px dashed #1a2e50", opacity:0.35 }}/>
              ...
              return (
                <div key={zone}
                  ...
                  style={{
                    ...
                    alignItems:"center", justifyContent:"center", gap:"4px", overflow:"hidden",
                  }}
                  ...
                  {(() => {
                    ...
                    const CHIP_SIZE = isMobile ? 22 : 34;
                    const CHIP_V_OFFSET = isMobile ? 7 : 10;
