...
export default function ProfilePage({
  ...
  return (
    <div style={{ maxWidth: "1080px", margin: "0 auto", padding: isMobile ? "0 10px 80px" : "0 20px 40px", fontFamily: "'Inter',sans-serif", overflowX: "hidden" }}>
      ...
      {(() => {
        const GAME_DEFS = [
          { name: T("gameDice"),      key: "dice_stats",     image: "/dice-card.jpg",        sub: T("manderOriginals"), bg: "#0d1e3a", accent: "#3a7aff" },
          { name: T("gamePlinko"),    key: "plinko_stats",   image: "/plinko-thumb.jpg",     sub: T("manderOriginals"), bg: "#2a0a0a", accent: "#e03020" },
          { name: T("gameKeno"),      key: "keno_stats",     image: "/keno-thumb.jpg",       sub: T("manderOriginals"), bg: "#18082e", accent: "#9b5cf6" },
          { name: T("gameMines"),     key: "mines_stats",    image: "/mines-card.jpg",       sub: T("manderOriginals"), bg: "#081e10", accent: "#22c55e" },
          { name: T("gameBlackjack"), key: "bj_stats",       image: "/blackjack-thumb.jpg",  sub: T("manderOriginals"), bg: "#1a1408", accent: "#f4a91f" },
          { name: T("gameHilo"),      key: "hilo_stats",     image: "/hilo-card.jpg",        sub: T("manderOriginals"), bg: "#081a28", accent: "#06b6d4" },
          { name: T("gameRoulette"),  key: "roulette_stats", image: "/roulette-card.jpg",    sub: T("manderOriginals"), bg: "#2a0810", accent: "#e01a50" },
          { name: T("gameBaccarat"),  key: "baccarat_stats", image: "/baccarat-card.jpg",    sub: T("manderOriginals"), bg: "#081428", accent: "#6366f1" },
        ];
        ...
        const SERVER_GAME_MAP: Record<string, string> = {
          ...
          "Roulette": "roulette_stats", "Baccarat": "baccarat_stats",
        };
      ...
      {(() => {
        ...
        const processBets = (bets: { game: string; amount: number; winAmount: number; createdAt?: string }[]) => {
          ...
          filtered.forEach(b => {
            ...
            if (!bestWin || profit > (bestWin.payout - bestWin.amount))
        ...
        if (serverStats && serverStats.recent_bets.length > 0) {
          ...
          if (statsRange === "all" && serverStats.biggest_win > 0 && serverStats.biggest_win_bet > 0) {
            ...
            if (!bestWin || serverStats.biggest_win > (bestWin.payout - bestWin.amount))
