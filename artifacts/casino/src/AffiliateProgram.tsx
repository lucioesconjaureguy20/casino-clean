...
interface Props {
  ...
  hideReferredTable?: boolean;
  onRegister?: () => void;
}

export default function AffiliateProgram({ username, t, dashboardOnly, hideReferredTable, onRegister }: Props) {
  const [tab, setTab] = useState<"overview" | "dashboard">(dashboardOnly ? "dashboard" : "overview");
  const [refCode, setRefCode] = useState(username.toLowerCase().replace(/[^a-z0-9]/g, "") || "");
  ...
  const fetchLink = useCallback(async () => {
    ...
    try {
      ...
      setRefCode(data.ref_code || username);
      const origin = window.location.origin;
      setRefLink(`${origin}/?ref=${data.ref_code || username}`);
    } catch {
  ...
  const ngrNum = parseFloat(stats.ngr);
  const monoCards = [
    { label: "Deposit Amount", val: fmt(stats.deposit_amount), raw: null, Icon: IconDollarCircle, hint: "Total amount deposited by your referred users during the selected period." },
    { label: "Bet Amount", val: fmt(stats.wager_amount), raw: null, Icon: IconDice, hint: "Total amount wagered by your referred users during the selected period." },
    ...
    <section style={{ maxWidth: "1080px", margin: "0 auto", padding: "0 20px 40px" }}>
      ...
      {tab === "overview" && (
        <div>
          ...
          {username && (
            <>
              <p style={{ color: "#e2e8f0", fontSize: "13px", fontWeight: 600, marginBottom: "12px" }}>Referral Link</p>
              <div style={{ ...cardStyle, marginBottom: "20px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", flexWrap: "wrap" }}>
                  <div style={{
                    flex: 1, minWidth: "200px",
                    background: "#0e1623", border: "1px solid #1e2a3d", borderRadius: "8px",
                    padding: "9px 14px", color: "#94a3b8", fontSize: "13px",
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                  }}>
                    {refLink || `${window.location.origin}/?ref=${refCode}`}
                  </div>
                  <button onClick={copyLink} className="aff-primary-btn" style={primaryBtn}>
                    <IconCopy size={14} color="#111" />
                ...
                a: (
                  <div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      <a href="https://t.me/Henry_mander" target="_blank" rel="noreferrer" className="aff-contact-link" style={{
                        ...
                        <div className="aff-contact-icon" style={{ width: "30px", height: "30px", borderRadius: "8px", background: "rgba(246,181,49,0.1)", border: "1px solid rgba(246,181,49,0.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.15s ease, border-color 0.15s ease" }}>
                          <IconTelegram size={15} color="#f6b531" />
                        </div>
                        <span><span style={{ color: "#94a3b8" }}>Telegram:</span> @Henry_mander</span>
                      </a>
      ...
      {tab === "dashboard" && (
        <div>
          ...
          {/* Referrals List */}
          {!hideReferredTable && <div style={{ ...cardStyle, marginBottom: "16px" }}>
            <p style={{ color: "#e2e8f0", fontSize: "13px", fontWeight: 600, margin: "0 0 14px" }}>
              Referred Players ({referrals.length})
            </p>
            {referrals.length === 0 ? (
              <div style={{ textAlign: "center", padding: "36px", color: "#556070", fontSize: "13px" }}>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: "10px", opacity: 0.35 }}>
                  <IconUsers size={36} color="#ffffff" />
                </div>
                No referred players yet. Share your link to start earning!
              </div>
            ) : (
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "12px" }}>
                  <thead>
                    <tr>
                      {["Player", "Joined", "Deposits", "Wagered", "NGR", "Commission"].map(h => (
                        <th key={h} style={{ textAlign: "left", padding: "8px 10px", color: "#94a3b8", fontWeight: 500, borderBottom: "1px solid #1e2a3d" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {referrals.map((r, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #0e1623" }}>
                        <td style={{ padding: "10px", color: "#e2e8f0", fontWeight: 500 }}>{r.referred_username}</td>
                        <td style={{ padding: "10px", color: "#94a3b8" }}>{new Date(r.created_at).toLocaleDateString()}</td>
                        <td style={{ padding: "10px", color: "#94a3b8" }}>{r.deposit_count || 0}</td>
                        <td style={{ padding: "10px", color: "#94a3b8" }}>{fmt(r.deposit_amount || "0")}</td>
                        <td style={{ padding: "10px", color: "#94a3b8" }}>{fmt(r.ngr || "0")}</td>
                        <td style={{ padding: "10px", color: "#f6b531", fontWeight: 600 }}>
                          {fmt(String(parseFloat(r.ngr || "0") * 0.15))}
                        </td>
                      </tr>
                    ))}
                  </tbody>
      ...
      {tab === "dashboard" && username && <RegisteredUsers referrer={username} />}
