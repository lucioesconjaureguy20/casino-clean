...
const MENTITAS_STATS: AffiliateStats = {
  clicks: 1566,
  signups: 1433,
  ftds: 954,
  ftd_conversion: "66.57",
  deposit_count: 954,
  deposit_amount: "795345.34",
  wager_amount: "1876452.21",
  ngr: "34235.00",
  commission_earned: "5135.25",
  commission_paid: "0.00",
  commission_balance: "5135.25",
  commission_rate: 0.15,
};

const MENTITAS_REFERRALS: Referral[] = [
  { referred_username: "Casilux9",    created_at: "2025-01-15T14:32:00Z", is_ftd: true, deposit_count: 5,  deposit_amount: "12450.75",  wager_amount: "420230.50",  ngr: "-2847.30" },
  { referred_username: "NightSlot88", created_at: "2025-02-03T09:17:00Z", is_ftd: true, deposit_count: 3,  deposit_amount: "5820.40",   wager_amount: "320180.25",  ngr: "718.60"   },
  { referred_username: "Ruleta7x",    created_at: "2025-02-20T18:55:00Z", is_ftd: true, deposit_count: 8,  deposit_amount: "78340.20",  wager_amount: "621450.80",  ngr: "7214.40"  },
  { referred_username: "VegasKing",   created_at: "2025-03-05T22:08:00Z", is_ftd: true, deposit_count: 2,  deposit_amount: "2190.60",   wager_amount: "215430.15",  ngr: "-577.25"  },
  { referred_username: "SlotPro23",   created_at: "2025-03-12T11:43:00Z", is_ftd: true, deposit_count: 4,  deposit_amount: "9750.90",   wager_amount: "87420.60",   ngr: "942.80"   },
  { referred_username: "GoldBet55",   created_at: "2025-03-28T16:20:00Z", is_ftd: true, deposit_count: 1,  deposit_amount: "890.30",    wager_amount: "47340.75",   ngr: "278.15"   },
  { referred_username: "LuckyAce",    created_at: "2025-04-02T07:55:00Z", is_ftd: true, deposit_count: 1,  deposit_amount: "340.50",    wager_amount: "6520.40",    ngr: "-118.70"  },
  { referred_username: "RoyalPlay",   created_at: "2025-04-10T20:30:00Z", is_ftd: true, deposit_count: 2,  deposit_amount: "380.50",    wager_amount: "1120.80",    ngr: "95.35"    },
];
...
export default function AffiliateProgram({ username, t, lang = "es", dashboardOnly, onRegister }: Props) {
  ...
  const [activeTooltip, setActiveTooltip] = useState<{key:string;rect:DOMRect;text:string}|null>(null);
  const [faqOpen, setFaqOpen] = useState<number | null>(null);
  const fetchLinkRetries = useRef(0);
  ...
  const fetchLink = useCallback(async () => {
    ...
    try {
      ...
      // Si es fallback (Supabase timeout/error):
      //   · Reintentar hasta 3 veces para asegurar que el código cargue
      //   · Después de 3 intentos fallidos, mostrar sin código
      if (data.fallback) {
        if (fetchLinkRetries.current < 3) {
          fetchLinkRetries.current += 1;
          willRetry = true;
          setTimeout(() => fetchLink(), 5000);
        }
        return;
      }
      fetchLinkRetries.current = 0;
      const code = data.ref_code || "";
      setRefCode(code);
      setCodeInput(code);
      if (code) setRefLink(`${window.location.origin}/?ref=${code}`);
    } catch {
      // Error de red: reintentar si aún quedan intentos
      if (fetchLinkRetries.current < 3) {
        fetchLinkRetries.current += 1;
        willRetry = true;
        setTimeout(() => fetchLink(), 5000);
      ...
      // Solo terminar estado de carga si no hay reintento pendiente
      if (!willRetry) setLoadingCode(false);
    }
  }, [username]);
  ...
  const fetchStats = useCallback(async () => {
    if (!username) return;
    if (username.toLowerCase() === "mentitaslots") {
      setStats(MENTITAS_STATS);
      setReferrals(MENTITAS_REFERRALS);
      setLoading(false);
      return;
    }
    setLoading(true);
    try {
      ...
      setStats(s && typeof s.signups === "number" ? s : getLocalStats(username));
    } catch {
  ...
  useEffect(() => {
    fetchLink();
    fetchStats();
    const interval = setInterval(fetchStats, 15 * 1000);
  ...
  const statCards = [
    { label: t("affVisits"), val: fmtNum(stats.clicks), Icon: IconEye, hint: t("affHint1") },
    { label: t("affSignUps"), val: fmtNum(stats.signups), Icon: IconUsers, hint: t("affHint2") },
    { label: t("affFTDs"), val: fmtNum(stats.ftds), Icon: IconDeposit, hint: t("affHint3") },
    { label: t("affFTDConv"), val: `${stats.ftd_conversion}%`, Icon: IconPercent, hint: t("affHint4") },
  ];

  // Monetization — for mentitaslots use MENTITAS_STATS totals directly so the
  // 8 visible sample players don't reveal the real aggregate sums.
  const isMentitas = username?.toLowerCase() === "mentitaslots";
  const ngrNum       = isMentitas ? parseFloat(MENTITAS_STATS.ngr)            : referrals.reduce((s, r) => s + parseFloat(r.ngr            || "0"), 0);
  const totalWager   = isMentitas ? parseFloat(MENTITAS_STATS.wager_amount)   : referrals.reduce((s, r) => s + parseFloat(r.wager_amount   || "0"), 0);
  const totalDeposit = isMentitas ? parseFloat(MENTITAS_STATS.deposit_amount) : referrals.reduce((s, r) => s + parseFloat(r.deposit_amount || "0"), 0);
  const monoCards = [
    ...
    { label: t("affNetWL"),
      val: (ngrNum >= 0 ? "+" : "-") + "$" + Math.abs(ngrNum).toLocaleString("es-ES", { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
    ...
    <section style={{ maxWidth: "1080px", margin: "0 auto", padding: "0 20px 40px" }}>
      ...
      {tab === "overview" && (
        <div>
          ...
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: "12px", marginBottom: "16px", alignItems: "stretch" }}>
            {/* Banner */}
            <div style={{
              flex: 1,
              borderRadius: "8px",
              ...
              position: "relative",
              backgroundImage: isMobile ? "url(/affiliate-banner-mobile.jpg)" : "url(/referral-banner.png)",
              ...
              <div style={{
                ...
                background: isMobile
                  ? "linear-gradient(to bottom, rgba(10,6,0,0.55) 0%, rgba(10,6,0,0.35) 60%, rgba(10,6,0,0.2) 100%)"
                  : "linear-gradient(to right, rgba(10,6,0,0.92) 0%, rgba(10,6,0,0.75) 35%, rgba(10,6,0,0.25) 65%, rgba(0,0,0,0) 100%)",
                borderRadius: "8px",
                pointerEvents: "none",
              }} />
              {/* Text block */}
              <div style={{
                ...
                {/* Title */}
                <div style={{
                  fontSize: isMobile ? "22px" : "clamp(22px, 2.8vw, 46px)",
                  fontWeight: 900,
                  color: "#FFFFFF",
                  ...
                  {(() => {
                    const words = t("affBannerTitle").split(" ");
                    const goldWord = t("affBannerGoldWord").toUpperCase();
                    if (isMobile) {
                ...
                <div style={{
                  fontSize: isMobile ? "12px" : "clamp(12px, 1.1vw, 16px)",
        ...
        <div>
          {activeTooltip && createPortal(
            <>
              ...
              />
              {/* Viewport-aware fixed tooltip rendered in document.body — never clipped by transforms */}
              {(() => {
                const tipLeft = Math.max(8, Math.min(activeTooltip.rect.left + activeTooltip.rect.width / 2 - 110, window.innerWidth - 228));
                const arrowLeft = Math.max(8, Math.min(activeTooltip.rect.left + activeTooltip.rect.width / 2 - tipLeft - 5, 204));
                return (
                  <div
                    style={{
                      position: "fixed",
                      zIndex: 99999,
                      width: "220px",
                      bottom: `${window.innerHeight - activeTooltip.rect.top + 8}px`,
                      left: `${tipLeft}px`,
                      background: "#0e1623",
                      border: "1px solid #1e2a3d",
                      borderRadius: "8px",
                  ...
                  >
                    {activeTooltip.text}
                    <div style={{ position: "absolute", bottom: "-5px", left: `${arrowLeft}px`, width: "9px", height: "9px", background: "#0e1623", border: "1px solid #1e2a3d", borderTop: "none", borderLeft: "none", rotate: "45deg" }} />
                  </div>
                );
              })()}
            </>,
                  ...
                  <div key={i} style={{ ...statCardStyle, position: "relative" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "10px" }}>
                      ...
                      <div style={{ position: "relative", flexShrink: 0 }}>
                        <button
                          onClick={(e) => { e.stopPropagation(); if (activeTooltip?.key === tKey) { setActiveTooltip(null); } else { setActiveTooltip({ key: tKey, rect: (e.currentTarget as HTMLButtonElement).getBoundingClientRect(), text: hint }); } }}
                          className="aff-info-btn"
                          style={{ background: "none", border: "none", padding: "0 2px", cursor: "pointer", color: activeTooltip?.key === tKey ? "#f6b531" : "#556070", lineHeight: 1, display: "flex", alignItems: "center", transition: "color 0.15s ease" }}
                        >
                          <svg viewBox="0 0 16 16" width="18" height="18" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <line x1="8" y1="7.6" x2="8" y2="10.8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
                          </svg>
                        </button>
                      </div>
                    </div>
      ...
      {tab === "dashboard" && username && <RegisteredUsers referrer={username} referrals={referrals} t={t} lang={lang} totalCount={username.toLowerCase() === "mentitaslots" ? 1433 : undefined} />}
