...
function InfoPanel({bet,onClose,lang}:{bet:number;onClose:()=>void;lang?:string}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.93)",zIndex:3000,overflowY:"auto",padding:"10px",display:"flex",alignItems:"flex-start",justifyContent:"center"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{width:"100%",maxWidth:"460px",background:"#0f1623",borderRadius:"10px",border:"1px solid #1e2a3e",overflow:"hidden",marginTop:"4px"}}>
    ...
    <>
      ...
      {showBetMenu&&!spinning&&(
        <div style={{position:"fixed",inset:0,zIndex:2000,display:"flex",alignItems:"flex-end",justifyContent:"flex-start",padding:"0 0 52px 4px"}} onClick={()=>setShowBetMenu(false)}>
          <div style={{background:"#131a28",border:"1px solid #2a3550",borderRadius:"10px",padding:"8px",minWidth:"130px"}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:"8px",color:"#64748b",fontWeight:700,letterSpacing:".6px",marginBottom:"6px",paddingLeft:"4px"}}>{T("cbApuesta")}</div>
            {BET_OPTS.map(b=>(
              <button key={b} onClick={()=>{setBet(b);setShowBetMenu(false);}} style={{display:"block",width:"100%",padding:"5px 10px",fontSize:"12px",fontWeight:800,background:bet===b?"linear-gradient(135deg,#f6b531,#ea9807)":"transparent",color:bet===b?"#000":"#94a3b8",border:"none",borderRadius:"6px",cursor:"pointer",textAlign:"left",marginBottom:"1px"}}>
                ${b<1?b:Math.round(b)}
              </button>
            ))}
          </div>
                ...
                <div style={{fontSize:"11px",fontWeight:900,color:"#f4a91f",lineHeight:1.2,textShadow:winMsg.includes("MEGA")?"0 0 20px #f4a91f":"none",display:"flex",alignItems:"center",justifyContent:"center",gap:"4px"}}>
                  {(()=>{
                    const emojiMap:[string,string,number][]=[
                      ...
                      ["🔥",assetUrl(`${import.meta.env.BASE_URL}emoji-bigwin.webp`),16],
                      ["⭐",assetUrl(`${import.meta.env.BASE_URL}emoji-freespins.webp`),16],
                    ];
                    for(const[em,src,sz] of emojiMap){
                      if(winMsg.startsWith(em))return<><img loading="lazy" decoding="async" src={src} style={{width:sz,height:sz,objectFit:"contain",flexShrink:0}}/><span>{winMsg.slice(em.length).trimStart()}</span></>;
                    }
