                        ...
                        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                          ...
                          {(u.balance_demo ?? 0) > 0 && (
                            <span style={{
                              ...
                              borderRadius: 6, padding: "2px 8px", fontSize: 12, color: "#86efac",
                            }}>
                              <strong style={{ color: "#4ade80" }}>{fmtBal(u.balance_demo!)}</strong> DEMO
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    >
                      <td style={td}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                          ...
                          {w.is_streamer_withdrawal && (
                            <span style={{ background: "#1e1530", border: "1px solid #7c3aed", borderRadius: 4, color: "#a78bfa", fontSize: 9, fontWeight: 700, padding: "1px 6px", letterSpacing: "0.04em" }}>STREAMER</span>
                          )}
                        </div>
                        {w.is_streamer_withdrawal && (
                          <div style={{ marginTop: 5, background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 6, padding: "5px 8px" }}>
                            <div style={{ fontSize: 10, color: "#f87171", fontWeight: 700 }}>⚠ Retiro de saldo Streamer</div>
                            <div style={{ fontSize: 9, color: "#94a3b8", marginTop: 2, lineHeight: 1.4 }}>Avisar al depto. de Partners para cortar la colaboración</div>
                          </div>
                        )}
                      ...
                      <td style={td}>
                        {w.last_deposit ? (() => {
                          ...
                          const isProfit = diff > 0; // casino ganó (depósito > retiro)
                          return (
                            <div>
...
function SupportTab({ token }: { token: string }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      loadChats(true);
      if (activeChatRef.current) loadMessages(activeChatRef.current.chat_id, false, true);
    }, 7000);
    return () => clearInterval(id);
  }, []);
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 16, height: 620 }}>
        {/* LEFT — Chat list */}
        <div style={{ width: 300, minWidth: 260, display: "flex", flexDirection: "column", background: "#111827", border: "1px solid #1e2a3d", borderRadius: 10, overflow: "hidden" }}>
        ...
        {/* RIGHT — Active chat */}
        <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#111827", border: "1px solid #1e2a3d", borderRadius: 10, overflow: "hidden" }}>
          {!activeChat ? (
...
interface Bet {
  ...
  multiplier: number; created_at: string;
}
