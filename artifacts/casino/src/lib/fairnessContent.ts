...
const DATA: Record<string, FGame[]> = {
  es: [
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo es un juego de multiplicador donde estableces un objetivo — desde 1,01x hasta 1.000.000x — y apuestas a que el resultado aleatorio lo alcanzará o lo superará. Cuanto más alto el objetivo, menor la probabilidad de ganar, pero mayor el multiplicador que cobras si sale.",
      sections:[
        { title:"Cómo se genera el resultado", body:"En cada apuesta, un generador de números aleatorios produce un valor entre 0 y 99.999.999. Ese valor se transforma en un multiplicador (mínimo 1,00x). Si el multiplicador resultante es mayor o igual a tu objetivo, ganás el multiplicador completo aplicado a tu apuesta." },
        { title:"Probabilidad de ganar", body:"La probabilidad de ganar es exactamente 99 ÷ multiplicador_objetivo. Por ejemplo, un objetivo de 2x tiene ~49,5% de probabilidad; un objetivo de 10x tiene ~9,9%; un objetivo de 100x tiene ~0,99%. El house edge del 1% se refleja en la fórmula y es fijo en todo momento." },
        { title:"Verificación independiente", body:"Cada resultado está determinado combinando la semilla del servidor, tu semilla de cliente y un nonce. El proceso es criptográficamente seguro e independientemente verificable: podés comprobar en cualquier momento que el casino no manipuló el resultado." },
      ],
      tags:["Multiplicador objetivo 1,01x–1.000.000x","Probabilidad = 99 ÷ objetivo","Resultado verificable on-chain"],
    },
  ],
  en: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo is a multiplier game where you set a target — from 1.01x up to 1,000,000x — and bet that the random result will reach or exceed it. The higher the target, the lower the win probability, but the bigger the multiplier you collect if it hits.",
      sections:[
        { title:"How the result is generated", body:"On each bet, a random number generator produces a value between 0 and 99,999,999. That value is converted into a multiplier (minimum 1.00x). If the resulting multiplier is greater than or equal to your target, you win the full multiplier applied to your bet." },
        { title:"Win probability", body:"Your win probability is exactly 99 ÷ target_multiplier. For example, a 2x target has ~49.5% chance; a 10x target has ~9.9%; a 100x target has ~0.99%. The 1% house edge is baked into the formula and remains constant at all times." },
        { title:"Independent verification", body:"Each outcome is determined by combining the server seed, your client seed, and a nonce. The process is cryptographically secure and independently verifiable — you can confirm at any time that the casino did not manipulate the result." },
      ],
      tags:["Target multiplier 1.01x–1,000,000x","Probability = 99 ÷ target","Verifiable on-chain result"],
    },
  ],
  pt: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo é um jogo de multiplicador onde você define um objetivo — de 1,01x até 1.000.000x — e aposta que o resultado aleatório o alcançará ou superará. Quanto mais alto o objetivo, menor a probabilidade de ganhar, mas maior o multiplicador que você recebe se acertar.",
      sections:[
        { title:"Como o resultado é gerado", body:"Em cada aposta, um gerador de números aleatórios produz um valor entre 0 e 99.999.999. Esse valor é convertido em um multiplicador (mínimo 1,00x). Se o multiplicador resultante for maior ou igual ao seu objetivo, você ganha o multiplicador completo aplicado à sua aposta." },
        { title:"Probabilidade de ganhar", body:"Sua probabilidade de ganhar é exatamente 99 ÷ multiplicador_objetivo. Por exemplo, um objetivo de 2x tem ~49,5% de chance; um objetivo de 10x tem ~9,9%; um objetivo de 100x tem ~0,99%. A vantagem da casa de 1% está incorporada na fórmula e permanece constante." },
        { title:"Verificação independente", body:"Cada resultado é determinado combinando a semente do servidor, sua semente do cliente e um nonce. O processo é criptograficamente seguro e verificável de forma independente — você pode confirmar a qualquer momento que o cassino não manipulou o resultado." },
      ],
      tags:["Multiplicador alvo 1,01x–1.000.000x","Probabilidade = 99 ÷ objetivo","Resultado verificável on-chain"],
    },
  ],
  de: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo ist ein Multiplikatorspiel, bei dem du ein Ziel festlegst — von 1,01x bis zu 1.000.000x — und darauf wettest, dass das zufällige Ergebnis es erreicht oder übertrifft. Je höher das Ziel, desto geringer die Gewinnwahrscheinlichkeit, aber desto größer der Multiplikator, den du erhältst, wenn es eintritt.",
      sections:[
        { title:"Wie das Ergebnis erzeugt wird", body:"Bei jeder Wette erzeugt ein Zufallszahlengenerator einen Wert zwischen 0 und 99.999.999. Dieser Wert wird in einen Multiplikator umgewandelt (mindestens 1,00x). Wenn der resultierende Multiplikator größer oder gleich deinem Ziel ist, gewinnst du den vollen Multiplikator, der auf deine Wette angewendet wird." },
        { title:"Gewinnwahrscheinlichkeit", body:"Deine Gewinnwahrscheinlichkeit beträgt genau 99 ÷ Ziel-Multiplikator. Zum Beispiel hat ein 2x-Ziel ~49,5% Chance; ein 10x-Ziel ~9,9%; ein 100x-Ziel ~0,99%. Der Hausvorteil von 1% ist in der Formel eingebaut und bleibt jederzeit konstant." },
        { title:"Unabhängige Verifizierung", body:"Jedes Ergebnis wird durch die Kombination des Server-Seeds, deines Client-Seeds und eines Nonce bestimmt. Der Prozess ist kryptografisch sicher und unabhängig verifizierbar — du kannst jederzeit bestätigen, dass das Casino das Ergebnis nicht manipuliert hat." },
      ],
      tags:["Ziel-Multiplikator 1,01x–1.000.000x","Wahrscheinlichkeit = 99 ÷ Ziel","Verifizierbares On-Chain-Ergebnis"],
    },
  ],
  fr: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo est un jeu à multiplicateur où vous définissez un objectif — de 1,01x jusqu'à 1 000 000x — et pariez que le résultat aléatoire l'atteindra ou le dépassera. Plus l'objectif est élevé, plus la probabilité de gagner est faible, mais plus le multiplicateur que vous encaissez est grand si ça arrive.",
      sections:[
        { title:"Comment le résultat est généré", body:"À chaque mise, un générateur de nombres aléatoires produit une valeur entre 0 et 99 999 999. Cette valeur est convertie en multiplicateur (minimum 1,00x). Si le multiplicateur résultant est supérieur ou égal à votre objectif, vous gagnez le multiplicateur complet appliqué à votre mise." },
        { title:"Probabilité de gagner", body:"Votre probabilité de gagner est exactement 99 ÷ multiplicateur_cible. Par exemple, un objectif de 2x a ~49,5% de chance ; un objectif de 10x a ~9,9% ; un objectif de 100x a ~0,99%. L'avantage de la maison de 1% est intégré dans la formule et reste constant en permanence." },
        { title:"Vérification indépendante", body:"Chaque résultat est déterminé en combinant la graine serveur, votre graine client et un nonce. Le processus est cryptographiquement sécurisé et indépendamment vérifiable — vous pouvez confirmer à tout moment que le casino n'a pas manipulé le résultat." },
      ],
      tags:["Multiplicateur cible 1,01x–1 000 000x","Probabilité = 99 ÷ cible","Résultat vérifiable on-chain"],
    },
  ],
  id: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo adalah permainan pengganda di mana kamu menetapkan target — dari 1,01x hingga 1.000.000x — dan bertaruh bahwa hasil acak akan mencapai atau melampauinya. Semakin tinggi target, semakin kecil kemungkinan menang, tetapi semakin besar pengganda yang kamu dapatkan jika berhasil.",
      sections:[
        { title:"Bagaimana hasil dihasilkan", body:"Pada setiap taruhan, generator angka acak menghasilkan nilai antara 0 dan 99.999.999. Nilai tersebut dikonversi menjadi pengganda (minimum 1,00x). Jika pengganda yang dihasilkan lebih besar atau sama dengan targetmu, kamu menang dengan pengganda penuh yang diterapkan pada taruhanmu." },
        { title:"Probabilitas menang", body:"Probabilitas menangmu adalah tepat 99 ÷ pengganda_target. Misalnya, target 2x memiliki ~49,5% peluang; target 10x memiliki ~9,9%; target 100x memiliki ~0,99%. Keunggulan rumah 1% sudah termasuk dalam rumus dan tetap konstan setiap saat." },
        { title:"Verifikasi independen", body:"Setiap hasil ditentukan dengan menggabungkan benih server, benih klienmu, dan nonce. Prosesnya aman secara kriptografi dan dapat diverifikasi secara independen — kamu dapat mengkonfirmasi kapan saja bahwa kasino tidak memanipulasi hasilnya." },
      ],
      tags:["Pengganda target 1,01x–1.000.000x","Probabilitas = 99 ÷ target","Hasil dapat diverifikasi on-chain"],
    },
  ],
  it: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo è un gioco a moltiplicatore dove imposti un obiettivo — da 1,01x fino a 1.000.000x — e scommetti che il risultato casuale lo raggiungerà o lo supererà. Più alto è l'obiettivo, minore è la probabilità di vincere, ma maggiore è il moltiplicatore che incassi se arriva.",
      sections:[
        { title:"Come viene generato il risultato", body:"Ad ogni scommessa, un generatore di numeri casuali produce un valore tra 0 e 99.999.999. Quel valore viene convertito in un moltiplicatore (minimo 1,00x). Se il moltiplicatore risultante è maggiore o uguale al tuo obiettivo, vinci il moltiplicatore completo applicato alla tua scommessa." },
        { title:"Probabilità di vincita", body:"La tua probabilità di vincita è esattamente 99 ÷ moltiplicatore_obiettivo. Ad esempio, un obiettivo di 2x ha ~49,5% di probabilità; un obiettivo di 10x ha ~9,9%; un obiettivo di 100x ha ~0,99%. Il vantaggio della casa dell'1% è incorporato nella formula e rimane costante in ogni momento." },
        { title:"Verifica indipendente", body:"Ogni risultato è determinato combinando il seed del server, il tuo seed client e un nonce. Il processo è crittograficamente sicuro e verificabile in modo indipendente — puoi confermare in qualsiasi momento che il casino non ha manipolato il risultato." },
      ],
      tags:["Moltiplicatore obiettivo 1,01x–1.000.000x","Probabilità = 99 ÷ obiettivo","Risultato verificabile on-chain"],
    },
  ],
  ko: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo는 1.01x에서 1,000,000x까지 목표를 설정하고 무작위 결과가 그에 도달하거나 초과할지 베팅하는 배수 게임입니다. 목표가 높을수록 승리 확률은 낮아지지만 적중 시 받는 배수는 커집니다.",
      sections:[
        { title:"결과 생성 방식", body:"각 베팅에서 난수 생성기는 0에서 99,999,999 사이의 값을 생성합니다. 그 값은 배수(최소 1.00x)로 변환됩니다. 결과 배수가 목표 이상이면 베팅에 전체 배수가 적용되어 당첨됩니다." },
        { title:"승리 확률", body:"승리 확률은 정확히 99 ÷ 목표_배수입니다. 예를 들어 2x 목표는 ~49.5% 확률, 10x 목표는 ~9.9%, 100x 목표는 ~0.99%입니다. 1% 하우스 엣지는 공식에 내장되어 있으며 항상 일정합니다." },
        { title:"독립 검증", body:"각 결과는 서버 시드, 클라이언트 시드, 논스를 결합하여 결정됩니다. 프로세스는 암호학적으로 안전하고 독립적으로 검증 가능합니다 — 카지노가 결과를 조작하지 않았음을 언제든지 확인할 수 있습니다." },
      ],
      tags:["목표 배수 1.01x–1,000,000x","확률 = 99 ÷ 목표","온체인 검증 가능한 결과"],
    },
  ],
  nl: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo is een vermenigvuldigersspel waarbij je een doel instelt — van 1,01x tot 1.000.000x — en wedt dat het willekeurige resultaat dat bereikt of overtreft. Hoe hoger het doel, hoe kleiner de kans op winst, maar hoe groter de vermenigvuldiger die je ontvangt als het raak is.",
      sections:[
        { title:"Hoe het resultaat wordt gegenereerd", body:"Bij elke inzet genereert een willekeurige getallengenerator een waarde tussen 0 en 99.999.999. Die waarde wordt omgezet in een vermenigvuldiger (minimaal 1,00x). Als de resulterende vermenigvuldiger groter of gelijk is aan jouw doel, win je de volledige vermenigvuldiger toegepast op jouw inzet." },
        { title:"Winstkans", body:"Jouw winstkans is precies 99 ÷ doel_vermenigvuldiger. Bijvoorbeeld: een doel van 2x heeft ~49,5% kans; een doel van 10x heeft ~9,9%; een doel van 100x heeft ~0,99%. Het huisvoordeel van 1% zit ingebakken in de formule en blijft constant." },
        { title:"Onafhankelijke verificatie", body:"Elke uitkomst wordt bepaald door de server seed, jouw client seed en een nonce te combineren. Het proces is cryptografisch veilig en onafhankelijk verifieerbaar — je kunt op elk moment bevestigen dat het casino het resultaat niet heeft gemanipuleerd." },
      ],
      tags:["Doelvermenigvuldiger 1,01x–1.000.000x","Kans = 99 ÷ doel","Verifieerbaar on-chain resultaat"],
    },
  ],
  pl: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo to gra z mnożnikiem, w której ustawiasz cel — od 1,01x do 1 000 000x — i obstawiasz, że losowy wynik go osiągnie lub przekroczy. Im wyższy cel, tym mniejsze prawdopodobieństwo wygranej, ale tym większy mnożnik, który otrzymasz, gdy się uda.",
      sections:[
        { title:"Jak generowany jest wynik", body:"Przy każdym zakładzie generator liczb losowych tworzy wartość między 0 a 99 999 999. Ta wartość jest przeliczana na mnożnik (minimum 1,00x). Jeśli wynikowy mnożnik jest większy lub równy twojemu celowi, wygrywasz pełny mnożnik zastosowany do twojego zakładu." },
        { title:"Prawdopodobieństwo wygranej", body:"Twoje prawdopodobieństwo wygranej wynosi dokładnie 99 ÷ mnożnik_docelowy. Na przykład cel 2x ma ~49,5% szans; cel 10x ma ~9,9%; cel 100x ma ~0,99%. Przewaga kasyna w wysokości 1% jest wbudowana w formułę i pozostaje stała przez cały czas." },
        { title:"Niezależna weryfikacja", body:"Każdy wynik jest określany przez połączenie ziarna serwera, twojego ziarna klienta i nonce. Proces jest kryptograficznie bezpieczny i niezależnie weryfikowalny — możesz w każdej chwili potwierdzić, że kasyno nie manipulowało wynikiem." },
      ],
      tags:["Mnożnik docelowy 1,01x–1 000 000x","Prawdopodobieństwo = 99 ÷ cel","Weryfikowalny wynik on-chain"],
    },
  ],
  ru: [
    {
    ...
    {
      key:"Limbo", label:"Лимбо", color:"#9333ea", soon:false,
      how:"Mander Лимбо — это игра с множителем, где вы устанавливаете цель — от 1,01x до 1 000 000x — и ставите на то, что случайный результат достигнет её или превысит. Чем выше цель, тем меньше вероятность выигрыша, но тем больше множитель, который вы получите при попадании.",
      sections:[
        { title:"Как генерируется результат", body:"При каждой ставке генератор случайных чисел производит значение от 0 до 99 999 999. Это значение преобразуется в множитель (минимум 1,00x). Если результирующий множитель больше или равен вашей цели, вы выигрываете полный множитель, применённый к вашей ставке." },
        { title:"Вероятность выигрыша", body:"Ваша вероятность выигрыша составляет ровно 99 ÷ целевой_множитель. Например, цель 2x имеет ~49,5% шанс; цель 10x — ~9,9%; цель 100x — ~0,99%. Преимущество казино в 1% встроено в формулу и остаётся постоянным." },
        { title:"Независимая верификация", body:"Каждый результат определяется комбинацией серверного сида, вашего клиентского сида и нонса. Процесс криптографически защищён и независимо верифицируем — вы можете в любой момент подтвердить, что казино не манипулировало результатом." },
      ],
      tags:["Целевой множитель 1,01x–1 000 000x","Вероятность = 99 ÷ цель","Верифицируемый on-chain результат"],
    },
  ],
  tr: [
    {
    ...
    {
      key:"Limbo", label:"Limbo", color:"#9333ea", soon:false,
      how:"Mander Limbo, 1,01x'ten 1.000.000x'e kadar bir hedef belirlediğiniz ve rastgele sonucun buna ulaşıp ulaşmayacağını ya da aşıp aşmayacağını bahsettiğiniz bir çarpan oyunudur. Hedef ne kadar yüksekse kazanma olasılığı o kadar düşük, ancak isabet ederse aldığınız çarpan o kadar büyük olur.",
      sections:[
        { title:"Sonuç nasıl üretilir", body:"Her bahiste, bir rastgele sayı üreteci 0 ile 99.999.999 arasında bir değer üretir. Bu değer bir çarpana dönüştürülür (minimum 1,00x). Ortaya çıkan çarpan hedefinize eşit veya büyükse bahsinize uygulanan tam çarpanı kazanırsınız." },
        { title:"Kazanma olasılığı", body:"Kazanma olasılığınız tam olarak 99 ÷ hedef_çarpan'dır. Örneğin, 2x hedef ~%49,5 şansa sahipken; 10x hedef ~%9,9; 100x hedef ~%0,99 şansa sahiptir. %1 ev avantajı formüle yerleştirilmiştir ve her zaman sabit kalır." },
        { title:"Bağımsız doğrulama", body:"Her sonuç, sunucu tohumu, istemci tohumunuz ve bir nonce birleştirilerek belirlenir. Süreç kriptografik olarak güvenli ve bağımsız olarak doğrulanabilir — kumarhanenin sonucu manipüle etmediğini istediğiniz zaman onaylayabilirsiniz." },
      ],
      tags:["Hedef çarpan 1,01x–1.000.000x","Olasılık = 99 ÷ hedef","Doğrulanabilir on-chain sonuç"],
    },
  ],
};
