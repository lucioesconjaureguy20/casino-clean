...
const HILO_RTP = 0.99;
const MAX_SKIPS = 5; // max free skips per round — prevents skip-farming exploit
...
export default function HiloGame({
  ...
  currentUser, onRequestLogin, onGameActive, onInsufficientBalance,
}: HiloGameProps) {
  const T = (k: string) => gt(_lang, k);
  const [isMobile, setIsMobile] = useState(window.innerWidth < 768);
  const [mobileHeight, setMobileHeight] = useState(window.innerHeight - 4);
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (currW !== prevW) {
        prevW = currW;
        setMobileHeight(window.innerHeight - 4);
      }
    };
  ...
  // deck is NEVER restored from localStorage — future cards must not be readable by the client
  const [deck, setDeck]               = useState<Card[]>([]);
  const [currentCard, setCurrentCard] = useState<Card | null>(_savedSession?.currentCard ?? null);
  const [skipsLeft, setSkipsLeft]     = useState<number>(_savedSession?.skipsLeft ?? MAX_SKIPS);
  const [animKey, setAnimKey]         = useState(0);
  const [history, setHistory]         = useState<{ id:number; card: Card; guess: "higher"|"lower"|"skip"|null; won: boolean|null; mult?: number }[]>(_savedSession?.history ?? []);
  ...
  const [betUsd, setBetUsd]           = useState<number>(_savedSession?.betUsd ?? 3.00);
  ...
  // NOTE: deck is intentionally NOT saved — saving future cards in localStorage
  // would let players read deck[0] via DevTools to predict every next card.
  // On restore the deck is always regenerated fresh (a new shuffle).
  useEffect(() => {
    if (phase === "playing" && currentCard) {
      try {
        localStorage.setItem(HILO_SAVE_KEY, JSON.stringify({
          phase, currentCard, history, betUsd, accMult, guessCount, skipsLeft,
          histIdCounter: histIdRef.current,
          // deck is deliberately omitted
        }));
      } catch {}
    }
  }, [phase, currentCard, history, betUsd, accMult, guessCount, skipsLeft]);
  ...
  const [statsPos, setStatsPos]     = useState(() => {
    ...
    return mob ? { x: Math.max(5, Math.round((window.innerWidth - 280) / 2)), y: Math.round(window.innerHeight * 0.28) } : { x: 310, y: 180 };
  });
  const [chartHover, setChartHover] = useState<number|null>(null);
  const isDraggingStats             = useRef(false);
  const statsDragOffset             = useRef({ x:0, y:0 });
  const statsPinchDist              = useRef<number|null>(null);
  const statsPinchScale             = useRef(1);
  ...
  const canBet     = betNum > 0 && (!currentUser || betUsdVal <= balance);
  ...
  function startGame() {
    ...
    if (betNum > 0 && !canBet) { onInsufficientBalance?.(); return; }
    ...
    setHistory([{ id: firstId, card: firstCard, guess: null, won: null, mult: 1.0 }]);
    setAccMult(1.0);
    setGuessCount(0);
  ...
  function skipCard() {
    ...
    if (skipsLeft <= 0) return; // skip limit reached
    setSkipsLeft(n => n - 1);
    playCardSwoosh();
    ...
    setLatestHistId(-1);
    setAnimKey(k => k + 1);
    const skipMult = accMult;
    setHistory(prev => {
      const upd = [...prev];
      upd[upd.length - 1] = { ...upd[upd.length - 1], guess: "skip", won: null };
      upd.push({ id: newId, card: nextCard, guess: null, won: null, mult: skipMult });
      return upd;
    });
  ...
  function guess(dir: "higher" | "lower") {
    ...
    setDeck(d.slice(1));
    setAnimKey(k => k + 1);
    const newAccMult = won ? accMult * mult : accMult;
    const lossId = won ? -1 : histIdRef.current++;
    setHistory(prev => {
      const upd = [...prev];
      upd[upd.length - 1] = { ...upd[upd.length - 1], guess: dir, won };
      if (won)  upd.push({ id: newId,   card: nextCard, guess: null, won: null,  mult: newAccMult });
      if (!won) upd.push({ id: lossId,  card: nextCard, guess: null, won: false, mult: 0 });
      return upd;
    });
  ...
  function cashout() {
    ...
    setPhase("result");
    if (winTimerRef.current) clearTimeout(winTimerRef.current);
    winTimerRef.current = setTimeout(() => setOutcome(null), 2500);
  }
  ...
  if (isMobile) {
    ...
    return (
      <div className="hilo-root" style={{ background:"#0e1320", display:"flex", flexDirection:"column", userSelect:"none", WebkitUserSelect:"none", height:`${mobileHeight}px`, overflow:"hidden" }}>
        ...
        {/* Win popup */}
        {outcome === "win" && (
          <div style={{ position:"fixed", top:"50%", left:"50%", transform:"translate(-50%,-50%)", zIndex:200, pointerEvents:"none" }}>
        ...
        {/* ── Middle area — compact/fixed, no scroll ── */}
        <div style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column", justifyContent:"center" }}>

        {/* ── CARD + SCALE + BUTTONS row ── */}
        <div style={{ display:"flex", alignItems:"flex-start", gap:"10px", padding:"16px 14px 10px", flexShrink:0 }}>

          {/* Left: Card stack + multiplier badge */}
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"8px", flexShrink:0 }}>
          <div style={{ position:"relative", width:`${cardW}px`, height:`${cardH}px` }}>
            {deckLayers.map(offset => (
            ...
            {exitCard && (
              <div style={{ position:"absolute", top:0, left:0, zIndex:3, pointerEvents:"none" }}>
                <HiloCard card={exitCard} size="medium" animKey={`exit-${animKey}`} animType={exitAnimType}/>
              </div>
            )}
            <div style={{ position:"relative", zIndex:2 }}>
              <HiloCard card={phase==="idle" ? null : currentCard} size="medium" animKey={animKey} animType={lastAction} showBack={showingBack} lose={outcome==="lose"}/>
            </div>
            {/* Skip button — top-right corner overlay on the card */}
            <button
              onClick={skipCard}
              disabled={!isPlaying || actionBusy || skipsLeft <= 0}
              title={skipsLeft > 0 ? `Saltar (${skipsLeft} restantes)` : "Sin saltos disponibles"}
              style={{
                ...
                boxShadow: (isPlaying && !actionBusy && skipsLeft > 0) ? "0 2px 8px rgba(0,0,0,.5)" : "none",
          ...
          <div style={{ flex:1, display:"flex", flexDirection:"column", gap:"10px", minWidth:0, alignSelf:"flex-start" }}>
            {/* Mayor o Igual */}
            <button
              ...
              style={{
                ...
                transition:"background .15s",
                boxShadow: (isPlaying && !actionBusy) ? "0 0 0 1px #1e3a28" : "none",
              }}
            >
            ...
            <div ref={histScrollRef} style={{ display:"flex", gap:"8px", overflowX:"auto", paddingBottom:"4px", scrollbarWidth:"thin", scrollbarColor:"#1e3050 transparent" }}>
              {history.map((h, i) => {
                ...
                const badgeText    = isFirst      ? T("hiloStartCard")
                  : isRevealLoss   ? "0.00x"
                  : (h.mult != null ? h.mult.toFixed(2) + "x" : "—");

                return (
                  <div key={i} style={{ flexShrink:0, position:"relative", display:"flex", flexDirection:"column", alignItems:"center", gap:"5px" }}>
                    <div style={{ position:"relative" }}>
                      <HiloCard card={h.card} size="small" animKey={`hist-${h.id}`} animType={h.id===latestHistId?"histFlipRight":"small"}/>
                      {h.guess && h.guess !== "skip" && (
                        <div style={{
                          position:"absolute", right:"-12px", top:"38%", transform:"translateY(-50%)", zIndex:10,
                          background: h.guess==="higher" ? "#14532d" : "#7f1d1d",
                          border: h.guess==="higher" ? "1px solid #22c55e66" : "1px solid #ef444466",
                          borderRadius:"5px", width:"20px", height:"20px", display:"flex", alignItems:"center", justifyContent:"center",
                          fontSize:"11px", fontWeight:900, color: h.guess==="higher" ? "#22c55e" : "#ef4444", lineHeight:1,
                        }}>
                          {h.guess==="higher" ? "↑" : "↓"}
                        </div>
                      )}
                    ...
                    {/* Mult badge below each card */}
                    <div style={{
                      background: badgeBg, borderRadius:"6px",
                      padding:"3px 8px", fontSize:"10px", fontWeight:700,
                      color:"#fff", whiteSpace:"nowrap", textAlign:"center",
                      minWidth:"52px",
                    }}>
                      {badgeText}
                    </div>
        ...
        <div style={{ flex:"0 0 auto", overflowY:"auto", overscrollBehavior:"contain", paddingTop:"14px", paddingLeft:"16px", paddingRight:"16px", paddingBottom:"20px", display:"flex", flexDirection:"column", gap:"9px", background:"#131a28", borderTop:"1px solid #1a2438" }}>
          ...
          {isIdle ? (
            <button onClick={startGame} disabled={!(betNum > 0) || isPlaying} style={{
              ...
              {(betNum > 0 && !canBet) ? T("insufficientBal") : T("hiloApostar")}
            </button>
          ) : (
          ...
          {/* Stats + Volume icon buttons — abajo a la izquierda */}
          <div className="hilo-icon-footer" style={{ display:"flex", gap:"8px", alignItems:"center" }}>
            <button
    ...
    <div style={{ width:"100%", height:"100%", position:"relative", userSelect:"none", WebkitUserSelect:"none" }}>
      <div className="game-ctrl-grid" style={{
        ...
        <div className="game-ctrl-panel" style={{
          ...
          {/* Bet amount group: label + input + error + quick buttons */}
          ...
          {(!isPlaying && (betNum <= 0 || !canBet)) ? (
            <div style={{ fontSize:"11.5px", color:"#e74c3c", fontWeight:600, paddingLeft:"2px" }}>
              {(!!currentUser && (balance <= 0.001 || (betNum > 0 && !canBet))) && <div>{T("insufficientBal")}</div>}
              {betNum <= 0 && <div>{T("bjMinBet")}{fmtMoney(0.01)}</div>}
            </div>
          ) : null}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"6px" }}>
            {[
              { label:T("btnMin"), action: minBet },
              { label:"½",   action: halveBet },
              { label:"2×",  action: doubleBet },
              { label:T("btnMax"), action: maxBet },
            ].map(b => (
              <button
                key={b.label}
                onClick={b.action}
          ...
          </div>{/* end bet amount group */}

          {/* Apostar / Retirar button */}
          {isIdle ? (
            <button
              onClick={startGame}
              disabled={!(betNum > 0) || !canBet}
              style={{
          ...
          {/* ── Volume & Stats footer ── */}
          <div className="hilo-icon-footer" style={{ marginTop:"auto", display:"flex", gap:"8px", position:"relative" }}>
        ...
        <div style={{
          ...
          {outcome === "win" && (
            <div style={{
              ...
              <div style={{
                ...
                borderRadius:"6px", padding:"20px 32px", textAlign:"center",
          ...
          <div style={{
            ...
            padding: isMobile ? "18px 12px 14px" : "48px 24px 20px", position:"relative",
            ...
            <div style={{
              ...
              paddingLeft: isMobile ? "8px" : "48px", paddingRight: isMobile ? "8px" : "48px",
              marginBottom:"20px",
            }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:"28px" }}>
                ...
                <div style={{ position:"relative", width: isMobile ? "92px" : "164px", height: isMobile ? "128px" : "230px", flexShrink:0 }}>
                  ...
                  <div style={{
                    ...
                    <img src="/mander-logo.png" alt="Mander" style={{
                      position:"relative", width: isMobile ? "58px" : "88px",
                      filter:"brightness(0) invert(1)", opacity:0.9, objectFit:"contain",
                    }}/>
                  </div>
                  {exitCard && (
                    <div style={{ position:"absolute", top:0, left:0, zIndex:3, pointerEvents:"none" }}>
                      <HiloCard card={exitCard} size={isMobile ? "medium" : "large"} animKey={`exit-${animKey}`} animType={exitAnimType} />
                    </div>
                  )}
                  <div style={{ position:"relative", zIndex:2 }}>
                    <HiloCard
            ...
            <div style={{
              width:"100%", display:"flex", gap: isMobile ? "8px" : "12px", marginBottom:"16px", marginTop:"10px",
                ...
                <div ref={histScrollRef} style={{
                  ...
                  {history.map((h, i) => {
                    ...
                    const badgeText    = isFirst      ? T("hiloStartCard")
                      ...
                      : (h.mult != null ? h.mult.toFixed(2) + "x" : "—");
                    return (
                      <div key={i} style={{ flexShrink:0, position:"relative", display:"flex", flexDirection:"column", alignItems:"center", gap:"8px" }}>
                        <div style={{ position:"relative" }}>
                          <HiloCard
                            card={h.card}
                            size="medium"
