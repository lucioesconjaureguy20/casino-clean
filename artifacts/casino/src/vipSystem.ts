...
export const VIP_RANKS: VipRank[] = [
  { name: "Bronze I",     tier: "Bronze",   minWager: 0,        nextWager: 500,      rakebackPct: 0.04,  color: "#cd7f32", gradient: "linear-gradient(135deg,#cd7f32,#8b4513)", rewardMin: 3,    rewardMax: 10,   image: "/ranks/bronze-i.svg"     },
  { name: "Bronze II",    tier: "Bronze",   minWager: 500,      nextWager: 2000,     rakebackPct: 0.045, color: "#cd7f32", gradient: "linear-gradient(135deg,#cd7f32,#8b4513)", rewardMin: 3,    rewardMax: 10,   image: "/ranks/bronze-ii.svg"    },
  { name: "Bronze III",   tier: "Bronze",   minWager: 2000,     nextWager: 8000,     rakebackPct: 0.05,  color: "#cd7f32", gradient: "linear-gradient(135deg,#cd7f32,#8b4513)", rewardMin: 3,    rewardMax: 10,   image: "/ranks/bronze-iii.svg"   },
  { name: "Silver I",     tier: "Silver",   minWager: 8000,     nextWager: 25000,    rakebackPct: 0.06,  color: "#c0c0c0", gradient: "linear-gradient(135deg,#c0c0c0,#808090)", rewardMin: 20,   rewardMax: 75,   image: "/ranks/silver-i.svg"     },
  { name: "Silver II",    tier: "Silver",   minWager: 25000,    nextWager: 60000,    rakebackPct: 0.07,  color: "#c0c0c0", gradient: "linear-gradient(135deg,#c0c0c0,#808090)", rewardMin: 20,   rewardMax: 75,   image: "/ranks/silver-ii.svg"    },
  { name: "Silver III",   tier: "Silver",   minWager: 60000,    nextWager: 125000,   rakebackPct: 0.08,  color: "#c0c0c0", gradient: "linear-gradient(135deg,#c0c0c0,#808090)", rewardMin: 20,   rewardMax: 75,   image: "/ranks/silver-iii.svg"   },
  { name: "Gold I",       tier: "Gold",     minWager: 125000,   nextWager: 250000,   rakebackPct: 0.09,  color: "#f4a91f", gradient: "linear-gradient(135deg,#f4a91f,#c07800)", rewardMin: 75,   rewardMax: 400,  image: "/ranks/gold-i.svg"       },
  { name: "Gold II",      tier: "Gold",     minWager: 250000,   nextWager: 500000,   rakebackPct: 0.10,  color: "#f4a91f", gradient: "linear-gradient(135deg,#f4a91f,#c07800)", rewardMin: 75,   rewardMax: 400,  image: "/ranks/gold-ii.svg"      },
  { name: "Gold III",     tier: "Gold",     minWager: 500000,   nextWager: 900000,   rakebackPct: 0.11,  color: "#f4a91f", gradient: "linear-gradient(135deg,#f4a91f,#c07800)", rewardMin: 75,   rewardMax: 400,  image: "/ranks/gold-iii.svg"     },
  { name: "Platinum I",   tier: "Platinum", minWager: 900000,   nextWager: 1500000,  rakebackPct: 0.12,  color: "#b8c8e8", gradient: "linear-gradient(135deg,#b8c8e8,#7080a0)", rewardMin: 400,  rewardMax: 2000, image: "/ranks/platinum-i.svg"   },
  { name: "Platinum II",  tier: "Platinum", minWager: 1500000,  nextWager: 2500000,  rakebackPct: 0.13,  color: "#b8c8e8", gradient: "linear-gradient(135deg,#b8c8e8,#7080a0)", rewardMin: 400,  rewardMax: 2000, image: "/ranks/platinum-ii.svg"  },
  { name: "Platinum III", tier: "Platinum", minWager: 2500000,  nextWager: 4000000,  rakebackPct: 0.14,  color: "#b8c8e8", gradient: "linear-gradient(135deg,#b8c8e8,#7080a0)", rewardMin: 400,  rewardMax: 2000, image: "/ranks/platinum-iii.svg" },
  { name: "Emerald I",    tier: "Emerald",  minWager: 4000000,  nextWager: 7000000,  rakebackPct: 0.15,  color: "#4dd890", gradient: "linear-gradient(135deg,#4dd890,#1a8850)", rewardMin: 1000, rewardMax: 7000, image: "/ranks/emerald-i.svg"    },
  { name: "Emerald II",   tier: "Emerald",  minWager: 7000000,  nextWager: 12000000, rakebackPct: 0.16,  color: "#4dd890", gradient: "linear-gradient(135deg,#4dd890,#1a8850)", rewardMin: 1000, rewardMax: 7000, image: "/ranks/emerald-ii.svg"   },
  { name: "Emerald III",  tier: "Emerald",  minWager: 12000000, nextWager: 12000000, rakebackPct: 0.17,  color: "#4dd890", gradient: "linear-gradient(135deg,#4dd890,#1a8850)", rewardMin: 1000, rewardMax: 7000, image: "/ranks/emerald-iii.svg"  },
];
...
export function canClaimInstant(user: string): boolean {
  ...
  // null → never claimed → can claim now
...
// ── Claim-window expiry ───────────────────────────────────────────────────────
// Weekly:  user has 7 days from unlock to claim (matches server period boundary)
// Monthly: user has 30 days from unlock, capped at start of next month
const WEEKLY_EXPIRY_MS  = 7  * 24 * 3600 * 1000;
const MONTHLY_EXPIRY_MS = 30 * 24 * 3600 * 1000;

function periodicExpiry(type: "weekly" | "monthly", unlock: number): number {
  if (type === "weekly") return unlock + WEEKLY_EXPIRY_MS;
  // Monthly: min(unlock + 30d, first of next month from unlock)
  const d = new Date(unlock);
  const nextMonth = Date.UTC(d.getUTCFullYear(), d.getUTCMonth() + 1, 1, 0, 0, 0);
  return Math.min(unlock + MONTHLY_EXPIRY_MS, nextMonth);
}

// ms remaining in the claim window (0 if not yet open or already expired)
export function msUntilPeriodicExpiry(type: "weekly" | "monthly", user: string): number {
  if (!user) return 0;
  const unlock = type === "weekly" ? getWeeklyUnlock(user) : getMonthlyUnlock(user);
  if (Date.now() < unlock) return 0;
  return Math.max(0, periodicExpiry(type, unlock) - Date.now());
}
...
export function canClaimPeriodic(type: "weekly" | "monthly", user: string): boolean {
  if (!user) return false;
  const unlock = type === "weekly" ? getWeeklyUnlock(user) : getMonthlyUnlock(user);
  if (Date.now() < unlock) return false; // not yet unlocked
  const exp = periodicExpiry(type, unlock);
  if (Date.now() >= exp) {
    // Claim window expired — forfeit balance, advance to next period
